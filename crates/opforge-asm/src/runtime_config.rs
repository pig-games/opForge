...
use vm::rollout::portable_expr_parser_runtime_enabled_for_family as vm_portable_expr_parser_runtime_enabled_for_family;
...
const OPFORGE_VM_EXPR_PARSER_OPT_IN_FAMILIES_ENV: &str = "OPFORGE_VM_EXPR_PARSER_OPT_IN_FAMILIES";
const OPFORGE_VM_EXPR_PARSER_FORCE_HOST_FAMILIES_ENV: &str =
    "OPFORGE_VM_EXPR_PARSER_FORCE_HOST_FAMILIES";
...
const LEGACY_OPTHREAD_EXPR_PARSER_OPT_IN_FAMILIES_ENV: &str =
    "OPTHREAD_EXPR_PARSER_OPT_IN_FAMILIES";
const LEGACY_OPTHREAD_EXPR_PARSER_FORCE_HOST_FAMILIES_ENV: &str =
    "OPTHREAD_EXPR_PARSER_FORCE_HOST_FAMILIES";
...
#[must_use]
pub fn expr_parser_opt_in_families_from_env() -> Vec<String> {
    let mut families = parse_family_list_from_env(OPFORGE_VM_EXPR_PARSER_OPT_IN_FAMILIES_ENV);
    for candidate in parse_family_list_from_env(LEGACY_OPTHREAD_EXPR_PARSER_OPT_IN_FAMILIES_ENV) {
        if !families
            .iter()
            .any(|existing| existing.eq_ignore_ascii_case(candidate.as_str()))
        {
...
#[must_use]
pub fn expr_parser_force_host_families_from_env() -> Vec<String> {
    let mut families = parse_family_list_from_env(OPFORGE_VM_EXPR_PARSER_FORCE_HOST_FAMILIES_ENV);
    for candidate in parse_family_list_from_env(LEGACY_OPTHREAD_EXPR_PARSER_FORCE_HOST_FAMILIES_ENV)
    {
        if !families
            .iter()
            .any(|existing| existing.eq_ignore_ascii_case(candidate.as_str()))
...
pub fn portable_expr_parser_runtime_enabled_for_family(
    ...
    vm_portable_expr_parser_runtime_enabled_for_family(
        family_id,
        opt_in_families,
        force_host_families,
    )
...
#[must_use]
pub fn portable_expr_parser_runtime_force_host_for_family(
    family_id: &str,
    force_host_families: &[String],
) -> bool {
    force_host_families
