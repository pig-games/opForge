...
use super::M68040CpuHandler;

pub struct M68040CpuModule;

pub const CPU_ID: CpuType = CpuType::new("m68040");
const CPU_ALIASES: &[&str] = &["68040", "mc68040"];

impl CpuModule for M68040CpuModule {
    ...
    fn register_encodings(&self) -> Vec<package::RegisterEncodingDescriptor> {
        crate::families::m68k::package_programs::m68020_register_encodings(CPU_ID.as_str())
    }

    fn register_ids(&self) -> &'static [&'static str] {
        crate::families::m68k::package_programs::M68040_CONTROL_REGISTER_IDS
    }
    ...
    fn form_mnemonics(&self) -> Vec<String> {
        ...
        forms.push("PFLUSH".to_string());
        forms.push("MOVE16".to_string());
    ...
    fn handler(&self) -> Box<dyn CpuHandlerDyn> {
        Box::new(M68040CpuHandler::new())
    }
}

impl CpuHandlerDyn for M68040CpuHandler {
    fn cpu_id(&self) -> CpuType {
        CPU_ID
    }
