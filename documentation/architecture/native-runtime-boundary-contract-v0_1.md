...
| Assembly driver | `opasm.amigaos.assembly_driver` | session/pass orchestration and subsystem callback dispatch | engine API, flow/text domain APIs, tkpkg bridge, event projection | directive strings, structural scans, operand request construction, selector adaptation, data/text/layout semantics after Items 5.8–5.9.4 |
...
| Expression service | Item 5.5.1 service over `opcore.amigaos.expr_bridge` | neutral-context adaptation and expression execution | expression bridge/runtime and neutral context only | direct engine table access, CLI output policy, package selection |
...
| 5.10 | expression bridge (decision-dependent) | retained bridge or proved narrow owner | none unless a split proves need | ownership decision and all remaining bridge state are explicit |
