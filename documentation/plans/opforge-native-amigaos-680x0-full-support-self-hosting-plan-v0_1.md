...
- [x] Item 22: prove native package execution for 68020 full-extension addressing
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact staged Item 22 set confirms that the native `xp1:` walker, recursive nonidentity-scale projection, and schema-v2 operand-record projection directly port Rust's architecture-neutral selector and operand-record contracts. The exact Rust-built package and all family definitions remain unchanged; operand paths, register classes, qualifiers, widths, indirection modes, opcodes, diagnostics, CPU availability, and legality remain package-owned. The post-wrapper correction only preserves Rust's equivalent parenthesized-tuple and prefix-call tuple spellings in the neutral indirect-container walker. Per the user's delegation constraint, Luna monitored test process lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-26): focused Level-B contracts pass for the complete full-extension corpus, Rust expression-path and recursive nonidentity-scale boundaries, schema-v2 operand-record layout, exact embedded-package equality, and the frozen native ownership boundary. Directed fresh Level-D guests prove direct, preindexed, postindexed, base/index-suppressed records, exact earlier-CPU rejection diagnostics, and explicit missing-input failure. The complete established native wrapper was invoked exactly once and re-proved every earlier Level-D group through the m68000 remaining-base shard (1388.36 seconds) and m68010 delta shard (555.22 seconds). Its final seven-case Item 22 aggregate attempted every case: all three m68000/m68010 negative cases passed, while the four positive m68020/m68030/m68040/canonical-alias cases exposed one shared neutral alias miss at `disp(base,index)`. Rust normalizes that spelling to the same indirect tuple as `(disp,base,index)`; the native walker was corrected accordingly, and only the strengthened directed expression-path test was rerun. Its fresh guest passed in 48.20 seconds with exact live-Rust bytes for both spellings plus the preindexed and postindexed forms, explicit guest completion/zero exit, and no orphan process. The broad wrapper was not repeated, as required by the Directed-Fix and Single Native Completion Quality Gate. The native formatter, CPU-specific architecture boundary, complete runtime-boundary inventory, staged native-porting gate, workflow gate, plan validators, and exact-index diff checks pass. No production Rust changed, so the Rust quality gate is intentionally not applicable.
...
- [x] Item 23: prove native package execution for the first 68020 later-family group
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact staged Item 23 set confirms that native direct/indirect call-argument register projection and distinct-register evaluation directly port Rust's architecture-neutral selector contracts. The exact Rust-built package and all family definitions remain unchanged; CPU availability, mnemonics, operand forms, register classes and encodings, opcodes, field meanings, diagnostics, and legality remain package-owned. Native code contains no 68020 mnemonic, register, opcode, or legality table, and CAS2 field legality remains explicitly deferred to Item 24. Per the user's delegation constraint, Luna monitored test process lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-26): all three focused Level-B Rust-boundary contracts pass for direct/indirect call-argument register projection, Rust's recursive distinct-register predicate, the frozen group-A rows and fixture, and exact embedded-package equality. The first directed CAS2.L guest localized one neutral parser-state defect: `tkpkgParseU16DecimalV2` clobbered the suffix length held in D5 while parsing argument zero. Preserving that length across the shared parser produced exact live-Rust bytes `0E FC 80 80 90 C1` with explicit guest exit zero. Two further fresh directed guests proved a distinct DIVS.L register pair byte-for-byte and the duplicate-pair Rust diagnostic with exits zero and one. The staged porting gate then exposed only a source-order mismatch in the test-name ledger; the ledger was reordered and its single ownership validator passed without repeating the broad gate. After every directed proof and deterministic contract passed, the complete established native wrapper was invoked exactly once and passed all 39 Level-D groups. It re-proved counted `.for`, macro, all earlier family and 68k paths, the 43-guest 68000 package corpus, the 30-case remaining-base aggregate (1390.67 seconds), the 12-case m68010 delta (554.25 seconds), the eight-case full-extension aggregate (368.20 seconds), and the final ten-case Item 23 group-A aggregate (463.48 seconds). Luna observed no lingering wrapper, cargo, test, or FS-UAE process. The 225-file native formatter, CPU-specific architecture boundary, complete runtime-boundary inventory, slice metadata, instrumentation, no-growth, proof-contract, ownership, and exact-index checks pass. No production Rust changed, so the Rust quality gate is intentionally not applicable.
...
- [x] Item 24: prove native package execution for the remaining 68020 and residual-risk group
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 24 set confirms that postfix-call scalar/member/indirect-tuple projection, relocation-target recognition, signed out-of-range rejection, and original-source directive-sigil disambiguation directly mirror Rust's architecture-neutral contracts. The Rust-built package and every family definition remain unchanged; CPU availability, mnemonics, forms, register classes, opcodes, bounds, legality, and diagnostic text remain package-owned. The driver consults original syntax only because native metadata removes the leading dot from `.pack`. Per the user's delegation constraint, Luna monitored process lifecycle and cleanup only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-26): focused Level-B contracts pass for every new neutral Item 24 projector, the exact frozen Rust rows and fixture, embedded-package equality, and the explicit-dot routing boundary. Directed fresh Level-D guests prove exact BFTST, BFEXTU, and BFINS bytes, relocation fixups, and the package-owned BFINS rejection. The complete established native wrapper was invoked exactly once: every earlier one of its 39 groups passed, including the 43-guest 68000 corpus, 30-case remaining-base aggregate, 12-case m68010 delta, eight-case full-extension aggregate, and ten-case Item 23 aggregate. Its final 14-case Item 24 aggregate attempted every case; 11 passed and three exposed two invariants: normalized bare `PACK` was consumed as `.pack`, and native lacked Rust's neutral `out_of_rangeN.minX.maxY` rejection projection for `CALLM #300`. The wrapper was not repeated. Only directed repairs were run afterward: two fresh guests proved bare PACK exact m68020 bytes and exact m68010 availability rejection (93.35 seconds), one fresh guest re-proved dotted `.pack` layout bytes (45.63 seconds), and one fresh guest proved the exact Rust CALLM range diagnostic with captured value and exit one (45.36 seconds). Luna confirmed no lingering cargo, test, launcher, or FS-UAE process. The 225-file native formatter, CPU-specific architecture boundary, runtime-boundary inventory, slice metadata, staged native-porting gate, workflow/plan validators, and exact-index checks pass. No production Rust changed, so the Rust quality gate is intentionally not applicable.
...
- [x] Item 25: implement frozen package-owned target-state operations in the native generic runtime
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 25 set confirms that the native state service generically interprets frozen `STVM` records and that the assembly driver only dispatches explicit dotted directives into that service. Source and CLI CPU selection, per-pass reset, profile overrides, directive argument normalization, selector state guards, and package diagnostics are transported without a native CPU, FPU, Apollo, register, opcode, or legality table. The embedded package remains byte-for-byte equal to the live Rust registry build. Per the user's delegation constraint, Luna monitored test lifecycle and cleanup only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-26): focused Level-B contracts pass for the `STVM` locator/loader, generic state-service surface, driver/pipeline/reset wiring, selector state guard, exact frozen Rust state descriptors, and embedded-package equality. Fresh Level-D guests prove exact live-Rust output for m68020 `.fpu 68881` and m68080 `.apollo on`, source `.cpu m68040` switching from a CLI m68020 selection, and deterministic rejection of the illegal m68020/68040 state pairs with explicit guest exits. After all directed Item 25 proofs and deterministic contracts passed, the complete established native wrapper was invoked exactly once. It re-proved the counted `.for`, macro, earlier directive/control-flow, expression, layout, family, 43-guest 68000 corpus, operator, percent-prefix, MOS6502, Intel8080, Motorola6800, and MOVE-control paths, then attempted every case in the 30-case m68000 remaining-base aggregate. Its sole failure was the previously supported duplicate-MOVEM diagnostic: Item 24's new preceding `out_of_range` prefix probe left its boolean result in D0, so the independent `duplicate_register` probe no longer received the source length. The wrapper was not repeated. One architecture-neutral length reload restores Rust's independent prefix checks; the focused Level-B dispatch contract and one fresh Level-D MOVEM duplicate-register guest now pass with the exact package-owned Rust diagnostic and explicit exit one. Luna confirmed lifecycle cleanup. Native formatter, CPU boundary, runtime-boundary inventory, workflow, slice, ownership, no-growth, proof-contract, and exact-index checks pass. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 26: prove native package execution for m68030/m68040 integer and shipped MMU behavior
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 26 set confirms that native consumes the frozen Rust-built m68030/m68040 selectors, register encodings, semantic programs, fixed programs, restriction rows, and diagnostics without adding a native CPU, mnemonic, control-register, opcode, or legality table. The only production repair is architecture-neutral: a zero-operand CTBL request now supplies Rust's canonical exact `implied` mode, so it cannot collide with the package-selected `semantic` TRAPNE placeholder. ADDA.W remains inherited package/VM functionality and is proved on both later CPUs. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-27): the Level-B boundary contract proves exact embedded-package equality, owned PFLUSH/MOVE16/MMU MOVEC/restriction rows, inherited ADDA.W, the Rust `semantic` TRAPNE placeholder, and architecture-neutral native ownership. Directed Level-D guests prove canonical/numeric 68030 alias selection, every unique carry-forward fixture form, exact zero-operand TRAPNE mode lookup plus its explicit-word control (91.58 seconds), and the shared unshipped PBcc/PDBcc family boundary with exact live-Rust diagnostics and explicit exit one (88.49 seconds). The legacy Rust frontend parses syntactically complete absent-family operands before instruction lookup while native stops at package-family absence, so those two diagnostic cases use the shared bare/single-register boundary; this is the documented frontend-order platform exception and does not change shipped package semantics. After all directed proofs passed, the complete established native wrapper was invoked exactly once and all 44 tests passed. It re-proved every earlier shard and then passed all 24 fresh Item 26 guests in 1044.10 seconds, including exact Rust bytes for the complete m68030/m68040 fixtures, both PFLUSH forms, all m68040 MMU MOVEC registers, both MOVE16 forms, and ADDA.W direct/immediate on both CPUs (`d0 c0 d2 fc 00 10`), plus exact restriction and unshipped-PMMU diagnostics. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 27: prove native package execution for the 68881/68882 core surface
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 27 set confirms that native consumes the frozen Rust-built m68020/m68030 external-FPU state guards, selectors, register encodings, semantic programs, fixed programs, and diagnostic templates without adding a native FPU target, mnemonic, register, format, opcode, or legality table. The three production repairs are architecture-neutral: an allowed state guard records its nested plan kind, neutral named `.name(arg0,arg1)` calls expose comma-separated arguments in Rust order, and a rejected state guard reloads the preserved selected mnemonic before rendering `{mnemonic}`. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-27): the Level-B boundary contract proves exact embedded-package equality, all four legal m68020/m68030 plus 68881/68882 state profiles, FP0-FP7 and FPCR/FPSR/FPIAR package encodings, representative core selector plans, and architecture-neutral native ownership. Directed Level-D proofs first isolated complete nested sequence execution (44.92 seconds), named-call `.pair(FP4,FP5)` projection (44.64 seconds), and four register/arithmetic, native-format, list/pair, and conditional/save-restore families (174.36 seconds). A four-case rejection probe then passed three Rust-exact illegal-FMOVE cases and exposed only a truncated disabled-FPU mnemonic; the neutral diagnostic fix was proved by one fresh guest with the complete Rust message and explicit exit one (44.72 seconds), and the four-case probe was not repeated under the Directed-Fix and Single Native Completion Quality Gate. After all directed fixes passed, the complete established native wrapper was invoked exactly once and all 45 tests passed. It re-proved every earlier shard and then passed all 16 fresh Item 27 guests in 713.21 seconds: both complete `68020_fpu_allmodes` and `68020_fpu_registers` fixtures plus conditional/save-restore forms across every legal m68020/m68030 and 68881/68882 pairing, followed by disabled-FPU and illegal data-register, PC-relative, and FPU-register effective-address diagnostics. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 28: prove native package execution for the remaining external-FPU catalog
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 28 set confirms that native consumes the frozen Rust-built external-FPU selectors, state guards, semantic programs, fixed programs, register encodings, and diagnostics for the remaining math catalog without adding a native FPU target, mnemonic, register, format, opcode, or legality table. No production implementation change was required: the existing architecture-neutral interpreter already executes the complete package-owned catalog. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-27): the Level-B boundary contract proves exact embedded/live package equality, all 27 external-only catalog selectors for m68020/m68030, their m68040 integrated-FPU rejection selectors, exact fixture presence and order, and absence of catalog vocabulary from generic native implementation paths. A directed Level-D proof passed two fresh guests in 90.21 seconds: the full external-only math/register source under m68020/68881 matched live Rust bytes exactly, and m68040/68040 FETOX matched Rust's integrated-FPU rejection exactly. After all directed proofs passed, the complete established native wrapper was invoked exactly once and all 46 tests passed. It re-proved every earlier shard and then passed all eight fresh Item 28 guests in 373.32 seconds: the complete `68020_fpu_instruction_catalog` external-only math surface across m68020/68881, m68020/68882, m68030/68881, and m68030/68882, followed by m68040/68040 FMOVECR, FSIN, and FETOX integrated-FPU rejections plus disabled-FPU FREM, all against live Rust artifacts or diagnostics with explicit guest exits. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 29: prove native package execution for the integrated 68040 FPU surface
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 29 set confirms that native consumes the frozen Rust-built m68040 integrated-FPU selectors, state guards, register encodings, semantic programs, fixed programs, and diagnostic rows without adding a native FPU target, mnemonic, register, format, opcode, or legality table. No production implementation change was required: the existing architecture-neutral interpreter executes the complete integrated surface. Illegal `.fpu` target transitions retain Item 25's documented boundary: the frozen STVM record owns the profile mask but carries no directive-specific diagnostic template, so native proves Rust's rejection decision with its deterministic generic STVM diagnostic while package selector diagnostics remain exact. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-27): the Level-B boundary contract proves exact embedded/live package equality, all integrated move/register/control-register, arithmetic/test, conditional, and save/restore selectors guarded for target 68040, package-owned disabled and external-only rejection rows, exact fixture coverage, and absence of FPU vocabulary from generic native implementation paths. A directed Level-D run proved the complete `68040_integrated_fpu` fixture byte-for-byte against live Rust and the exact integrated-68040 `FSIN` rejection; its incompatible `.fpu 68881` case exposed only an invalid expectation of Rust-identical directive prose, contradicting the already-governed Item 25 STVM boundary rather than native behavior. After correcting that proof contract, the same directed test passed all three fresh guests in 133.66 seconds with explicit exits. After all directed proofs passed, the complete established native wrapper was invoked exactly once and all 47 tests passed. It re-proved every earlier shard and then passed all five fresh Item 29 guests in 229.88 seconds: the complete integrated fixture with exact Rust bytes, exact disabled-FADD and integrated-FSIN package diagnostics, and Rust-equivalent illegal-target decisions for both 68881 and 68882. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 31: prove native package execution for the 68080 AMMX surface
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 31 set confirms that native implements only Rust's architecture-neutral expression-path, call-argument register-sequence, register-sequence relation, and register-index projection contracts. The unmodified Rust-built package remains the sole owner of the AMMX target, mnemonics, aliases, registers, shapes, pair/group constraints, encodings, availability, and diagnostics. The wrapper-discovered correction also follows Rust's selector construction exactly by matching every candidate row against each nonempty package shape alternative; it adds no CPU vocabulary or native instruction table. Per the user's delegation constraint, Luna was assigned test-lifecycle monitoring only and performed no analysis, fixes, or compliance judgment; macOS denied the subagent access to the root PTY and process list, so only the root-observed protocol assertions and exit results count as evidence.
  - Completion evidence (2026-08-28): the Level-B boundary contract passes against the exact live Rust package and proves the complete AMMX selector/record/diagnostic inventory plus Rust's expression-path and register-sequence algorithms. Directed fresh guests first localized the missing neutral projections, then proved exact TEX24.64, UNPACK1632, MINTERM, BFLYB, all four pair/group relation diagnostics, and the earlier-CPU rejection with live-Rust bytes or diagnostics and explicit exits. The staged native-porting gate passed its metadata, instrumentation, 23 proof-contract, invocation-policy, no-growth, architecture-boundary, ownership, and 228-file formatting checks before the completion run. The established native completion wrapper was invoked exactly once. It passed its first 34 shards with real nonzero test counts and fresh guest protocols, including the 43-guest Motorola68000 family corpus, then shard 35 attempted all seven movement/control guests: four passed and three exposed one selector-shape invariant, where a compact string-table index represented only one of Rust's expanded shape alternatives and allowed immediate syntax to fall through to a direct row. Native now preserves the alternative-scan cursor and compares each candidate row against Rust's requested shape. Per the Directed-Fix and Single Native Completion Quality Gate, neither the wrapper nor the not-yet-reached Item 31 aggregate was run again. Only the smallest closure proofs ran: `MOVE.W #$1234,D1` passed one fresh guest in 46.02 seconds with exact live-Rust bytes `32 3C 12 34` and guest exit zero; `LEA #1,A0` passed one fresh guest in 45.18 seconds with Rust's exact diagnostic and guest exit one. The focused Rust-boundary contract, composed native assembly surface, and two-test runtime-inventory validator pass after the correction. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 32: prove native package execution for the 68080 FPU surface
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 32 set confirms that native executes only architecture-neutral package contracts: three-register shape inference, register-index XOR projection, SEMV-v7 output-width handoff, and package-derived state-legality rendering. The exact unmodified Rust-built package remains authoritative for CPU/FPU targets, mnemonics, registers, shapes, encodings, fixups, legal pairings, and diagnostics. The generic state renderer contains no CPU/FPU names and reconstructs Rust's enum-target message only from the serialized directive, argument, active profile, and allowed-profile masks. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-28): the Level-B boundary contract passed against the exact live Rust package and proved the complete m68080 FPU selector/state/register/encoding/fixup/diagnostic inventory while generic native sources remained CPU-neutral. Directed Level-D proofs passed the complete checked-in FPU fixture byte-for-byte, exact `FDBNE.L` bytes `F2 48 00 0E 00 07`, default integrated state, disabled-state rejection, and illegal 68881 pairing. The three-case state run attempted every fresh guest and passed its first two, then exposed only generic wording for the illegal pairing; after the package-derived renderer was added, the first one-guest closure exposed a clobbered saved directive length as the visible truncation `FPU`, and the corrected one-guest rerun passed with Rust's exact message and explicit exit one. The staged deterministic gate passed its slice metadata, instrumentation, 23 contracts, invocation policy, no-growth/CPU boundary, evidence classification, ownership, fail-closed proof contracts, runtime inventory, and 228-file formatting checks; two static naming/inventory bookkeeping findings were corrected and each was rerun only through its smallest validator. The established native completion wrapper was then invoked exactly once. It passed 42 shards with real nonzero counts and fresh protocols, including the 43-guest Motorola68000 corpus, 30-guest remaining-base surface, and all prior state-transition proofs. Shard 43 attempted both illegal state-pair guests and stopped only because its test still asserted the old generic FPU wording. That tests-only expectation now uses the live Rust FPU diagnostic while retaining Item 25's governed generic Apollo diagnostic where STVM has neither a legal argument nor a diagnostic template; its focused two-guest rerun passed in 89.28 seconds with completed protocols and explicit exit one. Per the Directed-Fix and Single Native Completion Quality Gate, neither the wrapper nor the not-yet-reached 17-case Item 32 aggregate was rerun. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 33: prove unmodified Rust-built 680x0-only, 65x02-only, and combined package composition on native
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 33 set confirms that the m68k-only, MOS 65x02-only, and combined inputs are direct outputs of `build_hierarchy_package_from_registry`, both combined registration orders are byte-identical, and native neither transforms package bytes nor adds a family, CPU, instruction, register, selector, or state table. The production correction is architecture-neutral: native records the request pipeline, restores it after frontend tokenization, and selects it at each assembly pass before replaying package-owned `.cpu` transitions in source order, matching Rust's per-pass invariant. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment; macOS prevented it from reading the root PTY, so only root-observed test results count as evidence.
  - Completion evidence (2026-08-28): the Level-B boundary contract passed with exact Rust-builder sizes and FNV digests (`316375`/`3a1b84127ec7599a` for m68k-only, `61698`/`41ea86dc087431b3` for MOS-only, and `340973`/`415e3047bf52acbb` for combined), byte-identical combined registration order, bounded native capacity, exact family presence/absence, and CPU-neutral native ownership. Directed Level-D guests proved exact Rust bytes for all six m68k CPUs, all four MOS CPUs, repeated cross-family switching, and package-owned m68080 FPU reset after switching away and back; both missing-family cases completed with explicit exit one and `OTR004`. A host-only 65816 `.al` probe was rejected as outside the immutable package rather than hard-coded into native, and its replacement package-owned reset guest passed. The staged deterministic native gate passed its slice, instrumentation, 23 proof-contract, invocation-policy, no-growth/architecture-boundary, ownership, fail-closed, runtime-inventory, and 228-file formatting checks. The established native completion wrapper was invoked exactly once. Shards 1-46 each reported one passed and zero failed with real fresh-guest counts, including the complete MOS corpus, 43-guest Motorola corpus, 30-guest remaining-base surface, all CPU/FPU state paths, and the complete external-FPU catalog. Shard 47 then ran all five integrated-m68040 guests before only its obsolete post-run assertion expected the pre-Item-32 generic illegal-FPU wording; the guest cases themselves completed. The authoritative aggregate cases now require their live Rust diagnostics, and the smallest one-guest `.fpu 68881` closure passed in 46.28 seconds with fresh protocol completion, explicit guest exit one, and Rust's exact diagnostic. Per the Directed-Fix and Single Native Completion Quality Gate, the wrapper was not repeated and its unreached shards 48-50, including the Item 33 aggregate, were not run separately. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 34: make the complete Motorola 68000 reference corpus a mandatory native parity matrix
  ...
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 34 set confirms that the 43-source/79-artifact inventory is filesystem-derived, uncapped, and bound case-by-case to live Rust oracles. The two wrapper-discovered production corrections implement only Rust's architecture-neutral contracts: a package-declared bounded-register projection and propagation of engine-owned PC-backed label metadata into fixups. No CPU, mnemonic, register, addressing-mode, opcode, or diagnostic authority moved into generic native code. Per the user's delegation constraint, Luna monitored test lifecycle only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-28): the non-update Rust reference baseline passed one test and reproduced the checked corpus without fixture changes. Level B accounted for exactly 43 top-level sources and 79 artifacts: 36 `.srec`, 36 `.lst`, and 7 `.err`. The directed Level D proof ran two fresh guests: one binary case matched exact live Rust bytes with explicit exit zero and one diagnostic case matched the live Rust diagnostic with explicit exit one. The established completion wrapper was invoked exactly once. Shards 1-48 each reported one passed and zero failed with non-skipped execution. Shard 49 attempted all 18 AMMX guests and exposed two independent native invariants: `LOAD target(PC),E7` emitted an absolute label value instead of Rust's PC displacement, and valid `STOREM3 D0,D2,(A0)` lacked Rust's neutral `bounded_reg` projection. After the two smallest Rust-derived production fixes, only directed closure tests ran. The PC-relative closure completed one fresh guest with explicit exit zero and exact seven-byte Rust equality, including displacement `0004`; the bounded-register closure completed one fresh guest with explicit exit zero and exact Rust bytes `FE100225`. Per the Directed-Fix and Single Native Completion Quality Gate, the wrapper was not repeated; shards 50-51, including the new 43-case aggregate, were not reached, and that limitation is explicit rather than presented as aggregate execution. The recurring macOS sandbox/App-launch rule above remains binding: pre-guest `kLSNoExecutableErr` is invalid evidence and must not be retried in the same sandbox. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 36: complete native Hunk fixup, relocation, implicit-code, and diagnostic parity
  ...
  - Full quality gates: Shared Full Quality Gate; complete Rust Hunk compatibility matrix and unsupported-shape diagnostics; the fail-closed Level D `external_fs_uae_native_item36_*` matrix collectively owns the complete executable-Hunk proof. Under the Directed-Fix and Single Native Completion Quality Gate, do not add or run a second monolithic duplicate after every directed matrix case has passed and the established native completion wrapper has been invoked once.
  - FS-UAE execution note: every real guest starts with host application-launch permission. A filesystem-sandbox denial, `kLSNoExecutableErr`, or native compile failure before the fresh guest start marker is zero guest evidence; correct that pre-guest condition and rerun only the same directed case, never count it as a parity attempt or restart the single full wrapper.
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 36 set confirms that native transports Rust's package-owned portable absolute-fixup metadata, retains ordinary absolute bytes for BIN/listing artifacts, and substitutes section-relative addends only in the Hunk owner's copied payload. The Hunk writer mirrors Rust's emitted-section omission/remapping, target grouping, offset order, and deterministic unsupported-expression boundary. No package, family, CPU, mnemonic, register, addressing-mode, opcode, or diagnostic authority moved into generic native code; object/library/debug/symbol/overlay hunks remain out of scope. Per the user's delegation constraint, Luna monitored logs and process/test results only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-28): seven focused Level-B contracts passed. Directed fresh Level-D guests proved exact live-Rust Hunk bytes for explicit and implicit absolute-32 relocation, interleaved multi-target groups, `.long`, `.emit long`, empty-target omission/remapping, and all five accepted symbolic-long addend forms; three separate rejection guests completed with explicit exit one and their exact live-Rust Hunk diagnostics. The established native completion wrapper was invoked exactly once. Shards 1-34 passed with one test and zero failures each, including the 43-guest Motorola68000 corpus; shard 35 attempted all seven movement/control guests, six passed, and the qualified absolute JSR exposed that generic native assembly had written a Hunk section-relative addend into ordinary BIN output (`00` versus Rust `10` at byte 4). The wrapper was not repeated. The fix now restores the package's retained absolute encoded scalar after scratch-using metadata calls and moves section-relative substitution exclusively into flat/selected Hunk payload copies. One pre-guest native address-syntax failure counted as zero evidence; the next fresh guest exposed the scratch-restoration requirement; the final one-guest closure reported `runs=1`, `protocol_completed=true`, `guest_exit=Some(0)`, `rust_bytes=18`, `native_bytes=18`, `exact_match=true`, and absolute JSR bytes `[00, 00, 10, 10]`. Minimal Hunk-side rechecks then reported exact 84/84 selected-section bytes and 68/68 implicit-CODE bytes, both with relocation groups, completed protocols, and explicit exit zero. No production Rust changed, so no Rust full gate applies.
...
- [x] Item 37: implement complete native Motorola S-record output and multi-output routing
  - Source requirement or finding IDs: `N68X0-001`, `N68X0-005`, `N68X0-009`, `N68X0-010`.
  - Expected files: native S-record artifact owner, CLI `-s/--srec` and `-g/--go` parsing/state, sparse image-presence projection, multi-output routing, tests, one slice record. Rust's source-output registry does not support `format=srec`; native must reject it with the live Rust diagnostic instead of adding a native-only format.
  - Full quality gates: Shared Full Quality Gate; data/S7-S9, address-width, checksum, 32-byte line limit, sparse/sorted/last-write-wins behavior, empty-image termination, start address, simultaneous output, optional output path, and malformed-request tests. The directed `external_fs_uae_native_item37_*` matrix collectively owns Level D proof; do not add or run a duplicate monolithic aggregate after the matrix and the single established completion-wrapper invocation.
  - Rust authority note: the current `ImageStore::write_srec_file` emits only S1/S2/S3 data records and one matching S9/S8/S7 termination record. It emits neither S0 headers nor S5/S6 count records, so native must not invent them.
  - FS-UAE execution note: every real guest starts with host application-launch permission. A filesystem-sandbox denial, `kLSNoExecutableErr`, or native compile failure before the fresh guest start marker is zero guest evidence; correct that pre-guest condition and rerun only the same directed case, never count it as parity or restart the single full wrapper.
  - Plan-compliance review evidence: `PASS` — primary Sol review of the exact Item 37 set confirms native mirrors `ImageStore::write_srec_file`: package-neutral image bytes and written-address presence feed a dedicated artifact owner; CLI parsing/routing owns only Rust's `--srec`/`--go` contract; no CPU, family, dialect, mnemonic, register, addressing-mode, or opcode authority moved into generic native code. Rust's absent S0/S5/S6 records and unsupported source `format=srec` remain absent/rejected rather than becoming native-only behavior. Per the user's delegation constraint, Luna monitored logs and test outcomes only and performed no analysis, fixes, or compliance judgment.
  - Completion evidence (2026-08-29): the focused Level-B contract passed. Ten directed Level-D tests used fresh challenged guests and live in-memory Rust oracles: S1/S9, S2/S8 with 24-bit `--go`, S3/S7 with a 32+1 byte split, sparse-gap omission, sorted last-write-wins bytes, termination-only empty output, omitted-path derivation, simultaneous exact Hunk/S-record/listing artifacts, two exact malformed-`--go` diagnostics with explicit exit one, and exact source-`format=srec` rejection with explicit exit one. Every successful artifact test completed the protocol, exited zero, and matched Rust byte-for-byte. The staged native porting gate passed slice metadata, instrumentation, 23 contracts, FS-UAE policy, no-growth/CPU boundary, evidence classification, ownership, fail-closed proof contracts, runtime inventory, and the 232-file formatter. The workflow gate exposed only the expected changed engine-inventory and test-ownership ledgers; both directed ledger validators passed after refresh. The established 51-shard native completion wrapper was invoked exactly once: shards 1-50 passed with zero failures, including the 43-guest package corpus and all established CPU/FPU/family paths; shard 51 attempted all 43 complete-reference cases, 42 passed, and `68040_integrated_fpu.asm` alone timed out without a completion marker or process exit. The wrapper was not repeated. Its sole directed closure then ran that exact source in one fresh guest and reported `protocol_completed=true`, `guest_exit=Some(0)`, `rust_bytes=94`, `native_bytes=94`, and `exact_match=true`, proving a transient guest/launcher timeout rather than a semantic defect. The macOS filesystem-sandbox/application-launch rule above remains binding; `.git/index.lock` sandbox denial is host tooling evidence only and is retried with repository-index permission, never counted as product evidence. No production Rust changed, so no Rust full gate applies.
...
- [ ] Item 40: run and promote the terminal two-generation native self-build proof
  ...
  - Plan-compliance review evidence: harness-only `PASS` — primary Sol review confirms the exact staged tree, canonical logical argv/path mapping, captured-generation executable handoff, fresh per-generation challenges, explicit-exit requirements, in-memory artifact comparisons, and cleanup remain fail-closed. Terminal acceptance remains open because the once-only run did not complete generation 1 or start generation 2.
  - Execution evidence (2026-08-31): host harness contracts passed, including command identity, the 96-file <=30-byte AmigaOS staging map, captured-bootstrap challenge identity, and explicit rejection of empty bootstrap bytes. The terminal two-generation test was invoked exactly once. Its generation-1 guest timed out after 300 seconds without completion, explicit guest exit, or artifacts; generation 2 did not start and no parity was claimed. A permitted generation-one-only timeout check at 900 seconds failed identically, proving a native hang rather than an insufficient host timeout. Directed exact-module localization then completed fresh and exited one at statement 2058, `CMPI.B #';',D3`: the short operand snapshot treated the quoted semicolon as a comment, unlike Rust's parsed-expression boundary. Native now ends that snapshot only at an unquoted semicolon. The only final closure used one fresh guest for quoted `(` and `;` immediates, completed the protocol, exited zero, and matched the live Rust oracle exactly at 8/8 bytes (`0C0300280C03003B`). Per the Directed-Fix and Single Native Completion Quality Gate, the terminal test, generation-one product case, and exact-module probe were not repeated. Item 40 remains unchecked because neither native generation has supplied the required artifacts. No production Rust changed, so no Rust full gate applies.
  - Parking and resumption evidence (2026-09-01): Item 40 is intentionally parked until the separate native-performance plan has reduced full-build time enough for the terminal proof to complete practicably. Before parking, directed fresh-guest Level D closures proved the newly encountered Rust-authoritative invariants, including exact case-folded qualified `LEA` resolution and enclosing-scope retry for a sibling-block `BSR.W`; the latter completed with explicit guest exit zero and exact 8/8-byte equality (`610000044E754E75`). The canonical package-only Rust directory build completed in 29.14 seconds and produced a 554,144-byte Hunk, an 11-byte S-record, and a 10,096,156-byte listing. In contrast, the unchanged canonical native generation-1 build remained CPU-active under the reviewed A4000/68040 maximum-speed JIT profile for both two hours and six hours without guest DONE, guest exit, stdout/stderr diagnostic, or output artifacts; each runner therefore failed closed on timeout and no parity was claimed. A subsequent 24-hour-ceiling attempt was stopped when the user requested this parking checkpoint and is not test evidence. Generation 2 never started. Resume only after the performance work, from the committed state recorded here: run the exact terminal two-generation test with a deadline derived from measured optimized runtime, require both fresh challenged guests, explicit zero exits, and exact Hunk/S-record/list equality, then run the generation-2-first 53-test bonus wrapper once. Item 40 and Milestone 8 remain unchecked; no terminal acceptance criterion is waived or reinterpreted. No production Rust changed, so no Rust full gate applies.
