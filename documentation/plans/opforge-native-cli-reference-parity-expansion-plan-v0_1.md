...
- [x] Item 7: add the module, macro, and statement opcore parity shard
  ...
  - Closure evidence:
    ...
    - after Item 7.12 removed shared batch inputs and outputs, the final uncapped `native_reference_opcore_module_macro_statement_fs_uae` run on 2026-08-13 attempted and passed all 17 assigned roots in 379.54s; every case executed its own exact stored source from a freshly created mounted `Work:` volume, completed its fresh guest protocol with explicit zero exit, and matched only its own same-case in-memory Rust CLI bytes, including the unchanged `module_basics.asm` 4097-byte `.org`-spanning binary
...
- [x] Item 7.12: isolate every FS-UAE parity case's guest inputs and output evidence
  - Source requirement or finding IDs: `R1`, batch staging wrote every source/support tree to one shared `Work:` path before launch; `R2`, every result received the same final captured artifact map. Together these defects made the earlier multi-case Item 7 success insufficient proof. This item fully closes `R1` and `R2`.
  - Activation dependency: Item 7.11 and the parent Item 7 implementation are committed, but their aggregate Level D evidence must be treated as unproven until this remediation is green.
  - Invariant: every parity case runs in a separately created ephemeral mounted `Work:` volume containing its exact command-selected source, support files, command outputs, and fresh protocol directory. This keeps both relative outputs and explicit AmigaDOS `Work:` outputs case-local. Verification captures only that case volume and compares it only with the oracle carried directly by the actual case. The shared native executable and emulator configuration remain immutable infrastructure, never evidence. Guest support paths must be non-empty relative paths without absolute roots or parent traversal.
  - Expected files: `crates/opforge-asm/src/fs_uae_smoke.rs`; this plan's Item 7 closure evidence.
  - Full quality gates: focused runner isolation/unit tests; exact uncapped `native_reference_opcore_module_macro_statement_fs_uae`; separate `native_mos_forward_ref_stability_fs_uae`; `python3 scripts/workflow/run_native_porting_quality_gate.py --staged`; `RUST_TEST_THREADS=1 scripts/workflow/run_rust_quality_gate.sh`; `make workflow-gate`.
  - Validation evidence: 34 focused host tests stage distinct source bytes in distinct mounted case volumes, prove both override and built-in command-selected sources are actually staged, prove unsafe support paths are rejected, prove captured artifacts remain case-local, and prove every temporary tree is removed. An explicit root-qualified `Work:` Item 13 artifact proof passes under this same contract. The final corrected uncapped 17-root Level D run passes in 379.54s, and the separate forward-reference Level D proof passes in 22.39s; both use fresh protocols, explicit zero exits, exact same-case Rust oracles, and leave no FS-UAE evidence tree. The corrected diagnostic matrix attempted all 17 cases in 385.36s and failed closed on one genuine native diagnostic mismatch after the other 16 passed; that product mismatch is recorded as `I9-D-01` under Item 9 and is not weakened, excluded, or misreported as runner evidence.
...
- [x] Item 8.4: implement qualified logical-section-map layout parity
  ...
  - Implementation: the CLI preprocessor retains generic import section maps and supplies them through an architecture-neutral assembly-request callback after the opasm driver resets layout state. Statements and sections retain their structural module owner independently of mutable preprocessing state. Each map carries both the imported logical-section owner and the importing concrete-section owner into layout finalization, where source and target identities resolve as `(owner, section name)` rather than by a global plain-name first match. The map grammar also rejects a missing comma between entries. The finalizer distinguishes ordinary selected references from logical-section reachability, routes only the referenced logical unit into the mapped image, and preserves Rust's logical expression-visible label value while applying concrete placement only to output ordering and capacity.
  - Focused proof: `motorola68020_item84_native_use_section_map_is_retained_generically` passes the import-owner consumption, duplicate logical-section collision, malformed map-separator, callback, statement-owner, section-owner, and owner-qualified source/target lookup contract. The three imported-PC-label Level D boundary cases continued to pass during the correction runs, and the final exact stored `module_qualified_section_map.asm` Level D proof passed in 23.57s with fresh guest completion, explicit exit zero, and exact five-byte equality to its same-source in-memory Rust oracle.
  - Complete-shard evidence: the final uncapped 16-root Item 8 rerun attempted and passed every assigned root in 361.24s, including `module_qualified_section_map.asm`, `section_module_use_autoload.asm`, and `section_module_use_include.asm`. Every accepted root supplied fresh guest completion, explicit zero exit, and exact same-case Rust bytes; no incomplete emulator run was accepted. The two Item 8.5 roots turning green here is evidence for that next item, not completion of its still-required dedicated proof and review.
...
- [x] Item 9.R1: distinguish a known mnemonic from a supported selector form
  - Source requirement or finding IDs: `I9-D-01`, reproduced by the uncapped 17-case Level D diagnostic matrix on 2026-08-15 after all cases were attempted: `jmp $20,x` completed in the guest and failed with `OPC-NCLI025` unknown mnemonic instead of Rust's `OPC-NCLI026` unsupported addressing-mode diagnostic
  - First divergent boundary: generic native MSEL traversal sets its only match flag after mnemonic, active owner, and operand-shape matching all succeed, so a known mnemonic with no supported shape is indistinguishable from a mnemonic absent from the active package
  - Invariant: active-owner mnemonic recognition is retained independently of full selector-form matching; zero active-owner mnemonic matches reports unknown mnemonic, while one or more mnemonic matches with no selectable form reports unsupported addressing
  - Expected files: `native/motorola68000/amigaos/tkpkg/tkpkg_selection_service.asm`; focused generic selector-state contract in `crates/opforge-asm/src/tests/native_cli_diagnostic_routing.rs`; `documentation/plans/slices/native-porting-slice-cli-diagnostic-parity.toml`; this plan
  - Full quality gates: focused Level B selector-state contract; exact real unsupported-addressing Level D case through the named uncapped diagnostic matrix; native formatter; staged native-porting gate; workflow gate; full serial Rust quality gate
  - Validation evidence: `native_selector_retains_mnemonic_match_before_shape_rejection` proves the active-owner mnemonic bit is set before shape comparison and is the final unknown-versus-unsupported discriminator. The native CLI package assembles and the native 68000 formatter checks all 219 files. The final exact-index unrestricted, uncapped Level D diagnostic matrix attempted and passed all 17 cases in 381.89s with fresh guest protocol evidence; `unknown-mnemonic` remained `OPC-NCLI025`, while the exact stored `jmp $20,x` case completed with `OPC-NCLI026`. Two earlier attempts are explicitly invalid evidence: the first rejected all cases on a native build error, and the sandboxed launcher attempt rejected all cases after FS-UAE SIGABRT occurred before every fresh start marker.
  - Plan-compliance review evidence: `plan-compliance-reviewer` must return `PASS` for this one-invariant generic diagnostic-routing remediation before commit
...
- [x] Item 10.R0.1: restore overlapping and far-forward origin image parity
  - Source requirement or finding IDs: Item 10.R0 uncapped first run attempted all sixteen exact stored roots in 363.23s; fifteen passed, while `syntax.asm` completed freshly with guest exit 1 and statement 100 reported `OPC-NCLI027` because the native append-only 8192-byte image could not represent its `$2000`, `$4000`, `$1000`, `$F000` source-order origins
  - First divergent boundary: Rust's address-keyed `ImageStore` retains the lowest address, zero-fills gaps, and overwrites overlapping addresses, while native output used global append position and treated the first origin as permanently authoritative
  - Invariant: the flat image origin is the lowest address exposed by unsectioned output, an explicit origin, or a nonempty finalized placed concrete section; ordinary same-named concrete sections across module owners form one contiguous placed group in declaration order, differently named unplaced concrete statements route to discard, and logical/BSS sections do not establish the image origin; pass two writes every retained main-image statement at `current_pc - image_origin`, clearing only newly exposed forward gaps and overwriting backward/overlapping ranges by address; a zero-byte write exposes no address and mapped tails append at the main-image high-water mark
  - Expected files: native opasm image/driver owners; focused contract and exact stored-root tests; one slice metadata record; this plan
  - Full quality gates: focused Level B address-write contract; exact `syntax.asm` Level D proof; uncapped sixteen-root parent rerun; native formatter; staged native gate; workflow gate; full serial Rust gate
  - Implementation: pass one retains the minimum explicit or unsectioned emitted origin independently of the current source PC. Ordinary section placement now merges same-named concrete sections across module owners in declaration order, reserves their combined region size, and assigns each constituent a contiguous finalized base; this preserves Rust's ordinary section-name grouping for autoload/include roots while owner-qualified logical-map resolution remains separate. After layout finalization, every nonempty finalized placed concrete non-BSS section participates in the image-origin minimum, without treating the default zero PC, logical sections, differently named unplaced sections, or BSS reservations as emitted image evidence. The layout-owned candidate service preserves D3-D5 (including its D5 index and the driver's D4 minimum) by contract instead of relying on the broader `getSectionInfoV1` clobber set. The layout route discards remaining unplaced concrete section statements, matching Rust deferred-section emission instead of allowing a below-origin cursor underflow. Pass two initializes a per-statement write offset from `current_pc - image_origin`; main-image emission overwrites that address range, extends the retained image high-water mark, and zeroes only a newly exposed forward gap. Zero-byte emission returns before gap/high-water work, and mapped flush explicitly positions the main cursor at its high-water mark. The native artifact-count ABI now supports up to 65,535 bytes instead of the obsolete 8,192-byte localization boundary, so every assigned root—including the required 57,365-byte artifact—executes without exclusion.
  - Focused proof: `native_image_writes_follow_current_pc_across_overlapping_origins` passes the minimum-origin, placed-group, unplaced-discard, per-statement cursor, zero-byte, gap-clear, overwrite, mapped-flush, and full-span source contract. The final exact stored `native_syntax_overlapping_origins_fs_uae` run passed in 24.45s with fresh guest completion, explicit exit zero, and all 57,365 bytes equal to its same-source live Rust CLI oracle. The final strengthened four-case default-origin proof passed in 91.03s, including output before a later explicit origin and a mixed unplaced/placed 65C02 root whose Rust/native artifact is exactly the placed `$BB` byte. The final exact qualified-section-map proof passed all 5 bytes in 23.79s, and the two exact imported same-name section roots passed in 46.13s with all four bytes each.
  - Complete-shard evidence: after the first blanket-unplaced-discard attempt correctly failed two of sixteen layout roots after attempting all cases, ordinary same-name group placement restored those exact imported roots. The final uncapped `native_reference_opcore_syntax_expression_fs_uae` rerun attempted and passed all sixteen assigned roots in 364.16s, and the final affected `native_reference_opcore_layout_output_fs_uae` regression shard attempted and passed all sixteen assigned roots in 362.45s. No root was excluded, capped, or accepted from launcher status; every case required the shared fresh protocol, explicit zero exit, and its own exact live Rust artifacts.
  - Plan-compliance review evidence: `plan-compliance-reviewer` returns `PASS` for the generic image invariant and confirms the capacity is a 16-bit flat-image representation boundary rather than a test exclusion
