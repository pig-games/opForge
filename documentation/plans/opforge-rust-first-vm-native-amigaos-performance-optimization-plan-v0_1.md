...
- LSP source amendment (2026-09-05): user explicitly permits LSP to remain broken until the very end; all LSP repair and final qualification belong to Step 49 / Item LSP-close.
...
Work Items run from Step 8 onward. Historical prerequisites and phase gates do
...
**Current step: Step 40 of 49 · Item A-close — qualify early wins and clear the recorded failure debt.** Update this pointer and total when the active item or plan changes.
...
- **Step19 user-directed retention (2026-09-05):** the user explicitly values
  reducing source DOS calls from 1,608 to 8 and authorizes keeping this change.
  For A-source-buffer only, the measured 4.2048% focused runtime gain may remain
  below the ordinary 5% threshold; do not relabel that threshold as passed.
  Retention still requires exact focused binary output, unchanged native streams,
  verified call reduction and bounded read-ahead, the required quality gates and
  independent compliance. Both B10 timeouts remain incomplete; full B10 and
  stdout parity remain mandatory at A-close. No general threshold change follows.
...
- Every user-facing results report must explain how the last work contributed
  ...
  Report both the current step gain and total measured gain since the stated
  comparable baseline, separated by workload and Rust/native implementation.
  Never sum or compound unrelated step percentages; label a cumulative runtime
  gain unmeasured until matching baseline/current evidence exists.
...
- [x] Step 8 · Item 0f — Close the bounded investigation deliverable; retain failed qualification
  - Completion evidence (2026-09-05): `documentation/performance/results/opforge-item0f-completion-gates-2026-09-05.md` records explicit zero exits for the non-LSP Rust gate (1,586 assembler tests, 1,881.30s) and staged workflow gate (134 tests, 238 formatter files), 27 verified raw receipt hashes and the independently reviewed first-pause fix. Native qualification and LSP debt remain open under their named later owners. This checkbox is part of the focused Step 8 commit, not a claim of completed Phase A.
...
- [x] Step 9 · Item E0 — Classify failures relevant to the first native experiments
...
- [x] Step 15 · Item A-triage — Choose the next action from post-win evidence
  - Decision evidence (2026-09-05; final compliance and focused commit pending): `documentation/performance/results/opforge-step15-post-win-decision-2026-09-05.md` selects A-branch followed by one A-b10-localize item. Repeated host assembly remains an unmeasured candidate. That decision used labels 01–27. The later A-test-throughput amendment inserts the validation-cost experiment between A-branch and A-b10-localize; that amendment used sequence 01–28. The Step18 decision adds A-source-buffer at Step19; that decision used 01–29. Step19 adds A-include-stdout at Step20; that then-current sequence was 01–31, with LSP-close last. This checkbox closes only with the reviewed focused decision commit.
...
- [x] Step 16 · Item A-branch — Resolve the completed pass-two branch rejection
  ...
  - Steps and boundaries: Start with the exact schema case examples/mos6502/6502_first_run_artifact_contract.asm and its original command, CPU, package and live Rust oracle. Compare request/shape/selected-candidate state with crates/opforge-vm/src/execution_model/encoding_bridge.rs and native tkpkg_selection_service.asm / encode_eval_bridge.asm. Reproduce once at the original fixed bound, then permit at most one minimal discriminating observation using the approved native debug framework if source inspection is insufficient. Name the earliest divergent invariant before changing production code; repair that one owner only. CPU-specific semantics remain package-owned. If the current case already passes, prove the other recorded completed branch cases rather than invent a repair. If no causal boundary is established within this budget, stop observation and revise the plan with independent review; do not restart the same probe.
  - Before/after and stop/go: completed explicit-exit BEQ/BRA rejections are correctness evidence, not timeout attribution. Require fresh exact Rust artifact/exit proof for the corrected case and the related recorded branch cases. Do not claim that a branch fix resolves B10 or the all-modes timeout. A moved diagnostic is not a fix.
  - Gate tier: focused semantic repair.
  - Required focused gates: source/host boundary proof at its stated level; fresh Level D proof for all four recorded completed branch cases, preserving each original source/command/CPU/package/in-memory Rust oracle and attempt-all among those cases; whole schema/source-CPU aggregate and all 51 groups remain unchanged at A-close (scheduling, not suppression); native formatter, architecture/inventory and staged slice gates; workflow/plan validators.
  - Full quality gates: full non-LSP Rust before the code commit; all other required native and workflow commit gates remain mandatory. Broad unrelated 51-group rerun stays at A-close.
  - Plan-compliance review evidence: independent PASS naming the invariant, Rust/native boundaries, exact source/command/oracle, all attempts, gate receipts and remaining failure ownership.
...
- [x] Step 17 · Item A-test-throughput — Reduce repeated Rust validation cost if measured safe
  ...
  - Steps and boundaries: Record the unchanged control source/toolchain/environment and retained Step16 full non-LSP gate (1,591 assembler tests, 1,889.188706s whole). Use one existing costly host assembly test, native_platform_profile_harness_and_cli_assemble, for two warm control and two warm candidate runs with unchanged inputs, assertions and test selection. Measure candidate cold compilation separately. Trial only test-profile opt-level=1 with debug-assertions=true and overflow-checks=true, in a distinct target directory; do not clean/rebuild the control cache or change release/dev profiles. Capture verbose rustc evidence for effective optimization and both checks, including relevant test dependencies; reject inherited overrides that invalidate that proof. Do not skip or cache Rust oracles, native image assembly, tests, packages, diagnostics, formatter, clippy, audit, architecture or workflow checks. Do not run a competing benchmark/gate during timing. Stop after this one configuration; no optimization-level sweep.
  - Before/after and stop/go: The focused pair is only an early rejection discriminator, not whole-gate savings. Require exact test-result identity and output/reference assertions, no newly ignored tests or changed native skip conditions, and improvement greater than max(10%, observed warm variation). On that signal, run scripts/workflow/run_rust_quality_gate.sh --defer-lsp with the candidate profile and unchanged gate scope. Compare the complete package/test/result inventory against the same-source Step16 control, preserving all existing native environment skips as skips. Run one additional warm candidate locked non-LSP suite to check repeatability. Accept only with zero failures/differences, unchanged debug-assertion/overflow-check behavior, and measured warm savings that repay the measured candidate cold-build cost within the remaining Phase A validations; otherwise revert profile settings and commit an honest no-go. Report cold, focused and full costs separately. Never infer a product/native runtime speedup from this experiment.
  - Gate tier: bounded developer-loop performance experiment and retention decision.
  - Required focused gates: exact selected-test identities/results and source/toolchain/settings hashes; verbose compiler settings; complete non-LSP inventory comparison; negative assertion/overflow behavior probes in temporary code if compiler flags alone do not establish both properties; workflow/plan validators. Keep all failed attempts and timing receipts.
  - Full quality gates: complete non-LSP Rust quality gate for retained Cargo settings, plus required workflow and independent compliance. The full non-LSP gate remains mandatory for later Rust commits; this item changes compilation settings only. All native Phase A qualification and final LSP/full-workspace requirements remain unchanged.
  - Plan-compliance review evidence: independent PASS for identical coverage/oracles, effective checks, measured amortization, one-configuration budget and accepted/reverted disposition.
  - Commit outcome: one focused test-profile performance commit with evidence, or documentation-only rejection, before B10 harness changes. Host assembly caching remains deferred.
  - Definition of done: a measured, assertion-preserving validation speedup is retained, or the one candidate is explicitly rejected; no runtime optimization or Phase A closure is claimed.

- [x] Step 18 · Item A-b10-localize — Select one remaining B10 owner
  - Source requirement or finding IDs: SR-PAR, SR-MEAS, SR-EARLY, E0-3.
  - Expected files: existing corpus/profile runner and decoder only if needed; B10 evidence ledger, this plan and receipts. No production optimization is bundled here.
  - Dependencies/worktree: committed A-branch and A-test-throughput dispositions; fixed frozen B10 case and reviewed current integration binary.
  - Steps and boundaries: Run exactly one unchanged fixed-120s B10 observation using the minimum existing approved counters needed to distinguish source ingestion from statement/session work. Reuse the current runner and fail-closed protocol; do not restart the stopped 60/100s sampling loop, extend the timeout, shrink inputs, or collect a general profile. Source-reader frontend ownership and historical opcode counts are starting evidence, not a causal conclusion. The Step14 full-counter composition failed on valid module input; do not silently treat that observer as trustworthy. Select a validated minimal composition or record that instrumentation is the blocking boundary before running. Any proven harness defect gets a separately reviewed repair.
  - Before/after and stop/go: partial records are Level E localization only. Completion requires a fresh challenge, exact guest protocol, explicit zero exit and byte-for-byte live Rust parity. No speedup, parity or elapsed-time share follows from a timeout or call counts. After this single run, or a failed observer preflight that prevents launch, select one supported repair/measurement boundary and insert its independently reviewed commit-sized item before A-close, or record a precise blocker; no automatic retry.
  - Gate tier: bounded observation.
  - Required focused gates: provenance/decoder checks for the selected existing composition, exact run identity and cleanup evidence; workflow/plan validators and attempt receipt even on failure if launched; if preflight fails, preserve that failure receipt and stop without launch.
  - Full quality gates: mandatory full non-LSP Rust if Rust changes, plus applicable native/workflow gates; no unrelated broad native rerun.
  - Plan-compliance review evidence: independent PASS for observation sufficiency, one-run budget, proof level and the concrete next repair or unresolved boundary.
  - Commit outcome: one focused B10 localization/decision commit before any resulting repair starts.
  - Definition of done: one B10 causal owner or precise evidence boundary is recorded with an independently reviewed next action; A-close cannot start while B10 or any resulting repair remains incomplete.

  - Completion evidence (2026-09-05): `documentation/performance/results/opforge-step18-b10-minimal-observation-2026-09-05.md` and matching JSON preserve the exact minimal-observer preflight (fresh Level D module parity) and the sole fixed-120s B10 attempt. B10 completed the requested first-visit abort with fresh protocol, explicit exit 1, strict OFPR/OFIO decoding and zero overflow; this is successful controlled Level E localization, not full parity. Its 23,773 source DOS reads for 23,770 aggregate source bytes support A-source-buffer as the next trial; no extra checkpoint or retry is needed. Focused Rust and 11 Python contracts, full non-LSP Rust (252.75s), and workflow gates pass. No product speedup is claimed. This checkbox closes with the independently approved focused commit; all Phase A failure debt remains open.

- [x] Step 19 · Item A-source-buffer — Reduce source-stream DOS reads
  - Source requirement or finding IDs: SR-PAR, SR-MEAS, SR-EARLY, E0-3.
  - Expected files: native source reader and bounded reader state/constants, focused source-reader Rust harness, performance evidence and plan. Native 68000/parity/debug rule packs remain binding.
  - Dependencies/worktree: committed A-b10-localize; reuse its frozen B10 identity and current integration source. No new profiling framework or checkpoint is needed.
  - Steps and boundaries: Buffer only the existing source stream in `opforgeNativeCliTokenizeFileAtPath`, including resolved-module prefix consumption, with a default-off byte-reader reference switch. Preserve each suspended parent stream across include/module recursion, exact module start/end limits, CR/LF handling, EOF/error behavior, register/stack ABI and diagnostics. Keep bootstrap scanning and module discovery outside this slice. Rust reference is the actual source/include/module CLI path and fresh in-memory output oracle; generic native code must not acquire source-CPU semantics.
  - Evidence sufficient to try: Step18's single sealed Level E B10 record reached pass one with 1,350 statements and records 23,773 source DOS reads for 23,770 bytes, versus 16 module reads for 23,858 bytes. This names a repeated operation, not its time share or the sole timeout owner. Source inspection confirms one-byte reads in both normal consumption and resolved-prefix skipping.
  - Before/after and stop/go: Establish one exact focused complete parity pair covering refill boundaries and a suspended parent stream. After viability, complete a total of at least three matched completed alternating observer-off pairs on that same workload, counting the viability pair if its measurement boundaries match; use the approved minimal platform counters separately to prove fewer DOS calls. Preserve logical byte delivery; any read-ahead beyond a selected module range must be bounded and reported, never silently equated with consumed bytes. Retain only with exact parity, mechanism reduction and the plan's noise-aware performance criteria; otherwise revert or record the specific failure. Run frozen B10 control/candidate qualification under the unchanged bound, retaining failures as incomplete evidence; do not spend repeated diagnostic attempts or infer completion from Step18.
  - Gate tier: one production optimization and focused qualification.
  - Required focused gates: refill/EOF/CRLF and nested include/module resume, selected module extent and error behavior; classify host contracts separately from fresh Level D guest parity. Confirm counters with the approved decoder and all guest cleanup/protocol invariants.
  - Full quality gates: mandatory full non-LSP Rust, native formatter/staged checks, architecture boundary and workflow/plan validators. Keep broader current-native and B01–B10 qualification at A-close.
  - Plan-compliance review evidence: independent PASS for reader ownership, unchanged semantic boundaries, exact proof, matched measurements and retain/revert decision.
  - Commit outcome: one focused optimization/decision commit before A-close or another resulting repair.
...
- [x] Step 20 · Item A-include-stdout — Gate include records on explicit debug mode
  ...
  - Steps and boundaries: At the existing include-line report call, honor `state.NativeCliDebugEnabled` using the established native debug gate. Preserve the report routine and public exports. Rust reference is normal CLI stdout for the actual include source; native boundary is `line_processor::invocationPass` and the existing include report. No other output sites or performance primitives are bundled.
  - Evidence and proof: Step19 freshly observes the unwanted normal-mode line with exact binary output; source inspection connects the same path to B08's sibling includes. Historical B08 stream text was not retained, so do not invent it. Require fresh normal-mode exact output and empty stdout/stderr, an explicit debug-mode include case proving the report remains reachable with exact binary output and fresh zero exit, plus frozen B08 completion before any closure claim.
  - Gate tier: one focused correctness repair.
  - Required focused gates: source-order debug gate contract, exact normal/debug include guests and fresh frozen B08 under the unchanged timeout/protocol. Native assembly and debug/parity rule packs remain binding; no ad-hoc instrumentation.
  - Full quality gates: mandatory non-LSP Rust, native formatter/staged/architecture and workflow/plan validators; no broad native repetition before A-close.
...
- [x] Step 21 · Item A-mos-abs32 — Supply MOS absolute-long relocation semantics
  - Source requirement or finding IDs: SR-PAR, SR-TERM; B09 completed unresolved-label failure retained by A-close.
  - Expected files: MOS family package programs, generic directive fixup byte preservation, focused serialized-package/live Rust and native oracle tests, generated native CLI package, candidate manifest and evidence.
  - Dependencies/worktree: committed A-include-stdout; retain the warmed worktree and all historical corpus receipts.
  - Steps and boundaries: Frozen B09 transforms its source pointer to `.long data_start`. Native delegates this to the scoped `fix.abs32` role and deliberately rejects a symbolic target when the active package lacks that role. Supply the missing MOS-family role with existing neutral fixup VM v4, width four, little endian, value base, bit-pattern range, placeholder-zero unresolved policy and absolute relocation. Preserve the package-produced four bytes across the existing directive metadata callback instead of rebuilding them from a numeric value using native byte order. For explicit-section long directives, use the existing layout-normalized addend and the same package program with a non-target input to produce section-relative payload bytes without a second fixup. Mark only those retained fixups as payload-normalized so selected-section Hunk output preserves the encoded bytes. Flat and selected-instruction transport remain outside this slice. This restores the opaque-byte transport contract; no generic native CPU special case and no label-finality change. Rust reference is `.long` encoding and output-fixup construction in `line.rs`; native consumer is `emitPackageLongDirectiveForStatement`.
  - Evidence and proof: First reproduce unchanged B09 on the prior package. Require serialized MOS package execution to match live Rust cross-section `.long` bytes and relocation metadata, including a nonzero addend to discriminate byte order; require a focused fresh native exact BIN and relocation-bearing Hunk case, regenerate the shipped package and require fresh full B09 exact eleven-artifact disposition under the unchanged 120-second protocol. Any moved failure remains open and receives its own localized repair.
  - Package identity policy: This semantic repair intentionally changes candidate package bytes. Preserve the original corpus manifest and old package identity in history; retain exact source bytes, CLI arguments, artifact set and timeout. Record a separately named candidate manifest with new package/corpus hashes and explicit before/after package identity. Do not call the new package the original frozen package or compute a cross-package speedup. Subsequent runtime performance controls and candidates must use the same repaired package. A-close qualifies the original B01–B10 workloads under this repaired package, with historical failures preserved.
  - Completion evidence (2026-09-06; focused commit follows final artifact validation): fresh nonzero-addend BIN/Hunk Level D PASS; final B09 checks all11,8 exact with HEX/map/listing still failed. Non-LSP Rust PASS307.155961584s after correcting an explicitly transitioned package digest pin; first failed gate retained. Native/workflow and independent code/plan review PASS. New layout and HEX items retain resulting debt; Phase A remains open.
  - Gate tier: one package-owned correctness repair enabling qualification; no runtime gain claimed.
  - Required focused gates: serialized MOS/live Rust fixup oracle (Levels A/B), fresh unchanged-workload B09 (Level D) with explicit exit, challenge completion and exact artifacts; generated package consistency.
  - Full quality gates: mandatory non-LSP Rust, architecture/native formatter/staged and workflow/plan validators; independent plan compliance PASS before commit. CPU extension and native parity rule packs remain binding.
  - Plan-compliance review evidence: independent review of package ownership, byte order, exact artifact scope, package identity transition and gate receipts; no formal finding closure without its required reviewer.
...
- [x] Step 22 · Item A-layout-size — Preserve placed-section group size
  ...
  - Plan-compliance review evidence: independent register-lifetime, group-sum and exact-proof review before commit.
  - Commit outcome: one focused layout-size correction commit before A-hex-records or another resulting repair.
  - Definition of done: actual placed sizes/bases/region usage match Rust for the complete discriminator, or a precise failed boundary remains open; no full-map, B09 or Phase A claim while unrelated output differences remain.

- [x] Step 23 · Item A-hex-records — Preserve sparse addresses in native HEX output
  - Source requirement or finding IDs: SR-PAR, SR-TERM; remaining B09 HEX mismatch after A-mos-abs32.
  - Expected files: generic native output-artifact writer, focused HEX oracle tests, measured source snapshots and evidence.
  - Dependencies/worktree: committed A-mos-abs32 and A-layout-size; reuse the warmed native worktree and repaired package identity.
  - Steps and boundaries: Rust `ImageStore::write_hex_file` emits sorted present address entries, splitting at address gaps and the 32-byte record limit. Native `opasmOutputBuildHexArtifactV1` currently serializes the materialized BIN span with a 255-byte limit. Use the existing engine image-presence map to omit gaps and bound contiguous records to 32 bytes. Preserve flat BIN bytes and all other artifact owners; no CPU-specific semantics or new instrumentation.
  - Evidence and proof: The final Step21 B09 receipt shows one nine-byte native record versus Rust records for five bytes at $8000 and three bytes at $8006. Require live Rust oracle checks for that sparse source and a contiguous 33-byte boundary; require fresh exact native HEX parity and unchanged BIN for the focused source. Rerun full B09 once for exact eleven-artifact disposition; map/listing and compound-expression debt remain open until separately repaired and proven.
  - Before/after and stop/go: correctness qualification only; scan existing presence data linearly with no new capacity. No runtime improvement promised. Report step and cumulative performance separately; do not infer speed from fewer failed artifacts.
  - Gate tier: one bounded output-invariant repair.
  - Required focused gates: host sparse/32-byte record oracles (Levels A/B), fresh exact BIN/HEX guest proof (Level D), complete B09 disposition, formatter, architecture, source budgets and inventory.
  - Full quality gates: mandatory non-LSP Rust, staged native and workflow/plan validators; broad Phase A qualification remains at A-close.
  - Plan-compliance review evidence: independent review of present-address authority, bounded record scanning, exact oracle/guest scope, all receipts and remaining debt before commit.
  - Commit outcome: one focused HEX repair commit before another resulting repair or A-close.
  - Definition of done: sparse and 32-byte HEX record boundaries match Rust with required proof and review, or an explicit failed disposition names the next causal boundary. No B09 or Phase A success while other artifacts differ.

- [x] Step 24 · Item A-map-symbols — Render canonical symbols without a 32-label limit
  - Source requirement or finding IDs: SR-PAR, SR-TERM; B09 map naming/visibility mismatch and source-proven traversal truncation.
  - Expected files: native source-artifact symbol map writer, focused map oracles, source snapshots and evidence.
  - Dependencies/worktree: committed A-hex-records and A-layout-size; retain the repaired package and warmed worktree.
  - Steps and boundaries: Map output must emit canonical engine label names once, in Rust's stable ASCII-folded lexical order, with visibility determined by the matching recorded export identity. Current output adds a duplicate root prefix and compares canonical names against raw export names even though the export table separately retains the owner module. Compare identities through the existing owner-name and raw-name pools, without allocating a second name buffer. Replace the 32-bit visited mask, whose bit operations alias larger label indices, with a bounded successor traversal over all retained labels. Preserve deterministic tie behavior and existing ordering complexity without new capacity.
  - Evidence and proof: Rust authority is the symbol portion of `opforge-vm/src/output_artifacts.rs`; native authority is `mapAppendSymbolsV1` and `labelIsPublicV1`. Require a complete root-module source with at least35 zero-padded labels, public/private labels on both sides of index32, and exact live Rust `symbols=all` and `symbols=public` maps under fresh zero-exit guest proof. Bind raw export recording before later engine scope qualification. Scope first proof to root-module labels; do not claim imported/block-scope map parity without its evidence. Rerun unchanged B09 once for all eleven artifacts; listing and all other Phase A failures remain open.
  - Before/after and stop/go: remove duplicate formatting and the incorrect32-label ceiling; no runtime percentage promised. Keep traversal no worse than existing quadratic selection and no new state. Report step and cumulative performance separately.
  - Gate tier: one map-symbol identity/traversal invariant repair.
  - Required focused gates: Rust map oracle and native source/ABI checks (Levels A/B), exact all/public native map proof (Level D), B09 disposition, formatter, architecture, inventory and measured source/export budgets.
  ...
  - Plan-compliance review evidence: independent canonical-name, owner comparison, ordering/tie and traversal-termination review plus exact proof receipts before commit.
...
- [x] Step 25 · Item A-listing-sections — Render authoritative section annotations in listings
  ...
  - Full quality gates: mandatory non-LSP Rust, staged native and workflow/plan validators; broad qualification remains at A-close.
  - Plan-compliance review evidence: independent statement-to-section ownership, open/close semantics, buffer/ABI and exact proof review before commit.
...
- [x] Step 26 · Item A-listing-visibility-source-rows — Retain consumed visibility directives in listing source records
  - Completion evidence (2026-09-06; independent compliance PASS, focused commit follows): Step26 MD/JSON records exact complete native listing/BIN and focused Rust/source contracts PASS, full non-LSP Rust PASS and staged workflow PASS. Original visibility rows are retained without statements or package routing. B09 checks11 artifacts with10 exact; its next mismatch1260 is reservation row19, owned by Step27. No step or cumulative runtime gain is measured; Phase A remains open.
...
- [x] Step 27 · Item A-cpu-execution-contract — Carry authoritative CPU word size and address bounds into native execution
  ...
  - Steps and boundaries: Define and implement a target-neutral package execution contract carrying nonzero word-size bytes and maximum program address from the Rust CPU handler authority. Resolve aliases to canonical CPU metadata and preserve exact values when switching pipelines. Use an optional versioned execution-contract chunk keyed one-to-one to canonical CPUS entries; aliases resolve canonical records rather than duplicating properties. Keep legacy CPUS record boundaries unchanged. Review the exact wire shape and document older-reader behavior before editing the codec. Validate malformed, duplicate, dangling and incomplete records. Missing legacy metadata must remain distinguishable from a valid value: preserve unrelated legacy package behavior, and fail explicitly when a requested operation requires unavailable properties rather than silently guessing CPU/family values. Expose typed Rust and native accessors with explicit success/failure. Stage/cache the selected CPU properties once per selection, with no per-opcode name dispatch or added statement storage. Package VM/family definitions remain the authority for CPU-specific values; generic code must contain no CPU-name or family-specific switches.
  - Evidence and proof: Bind package generation to registry `native_word_size_bytes` and `max_program_address`, including a nonstandard three-byte-word test CPU and aliases. Round-trip and malformed-input tests must prove exact preservation, rejection and explicit missing-metadata behavior. Confirm equivalent Rust/native accessors for at least two differing property sets and an alias/CPU switch in a complete fresh native harness using the actual case's live Rust oracle and explicit zero guest exit; legacy/missing-property failure needs fresh completion and an explicit nonzero result. The native harness must use the real package loader/selection/accessors, not a reimplemented decoder or persisted oracle. Keep RES arithmetic and listing rendering for the next item.
  - Package identity policy: Preserve historical frozen package and corpus identities. Record a separately named candidate package/manifest with new hashes; keep original B01-B10 source bytes, commands, artifact sets and timeouts. Subsequent controls/candidates must use the same new package. No cross-package runtime gain is claimed; all corpus/native qualification remains required at A-close.
  - Before/after and stop/go: correctness prerequisite only; report step and cumulative gains as unmeasured unless matched timing exists. Bound ongoing cost to CPU selection and the small property cache; avoid a general metadata redesign. Stop with proven property transport, then commit before consuming it in RES.
  - Gate tier: one CPU execution-property transport invariant across Rust package and native selection.
  - Required focused gates: registry/package authority and codec rejection/alias tests (Levels A/B), fresh complete native loader/selection/accessor proof (Level D), native formatter, architecture, inventory and exact package/source budgets.
  - Full quality gates: mandatory non-LSP Rust quality gate, staged native and workflow/plan validators; broad Phase A qualification remains at A-close.
...
- [x] Step 28 · Item A-scalar-i64 — Preserve signed 64-bit scalar authority before reservation qualification
  - Completion evidence (2026-09-06; closes with this focused commit): `documentation/performance/results/opforge-step28-scalar-i64-2026-09-06.md` and its JSON distinguish fresh 414-case runtime, 29-value CLI bridge, 64 typed-service and four negative-process proofs from aggregate failures and host-only checks. All seven affected native regressions, the non-LSP Rust gate (1,626 assembler tests), staged native gate, ownership/inventory and measured capacity checks pass. Fixed BSS grows 54 B and constant data 152 B; measured capacities retain existing bounds. The common-digit fast path and constant-time paired shifts have no matched runtime measurement. Reservation remains a separate following item; no total runtime gain or Phase A closure is claimed.
  - Source requirement or finding IDs: SR-PAR; Step28 scalar-authority audit proves that low32 aliases valid positive high-bit values with negatives and hides computed overflow. The original general nonnegative-expression/full-u32 reservation requirement remains binding.
  - Expected files: native ExprVM runtime, EXVM literal bridge, expression service/result consumer boundary, focused scalar tests/harness, slice metadata, boundary inventory and measured budgets/evidence, plus the exact functional-error-output checker exception and its guard regression test.
  - Dependencies/worktree: committed A-cpu-execution-contract; preserve the warm target. The unfinished reservation draft is saved with exact file hashes and base commit at `/private/tmp/opforge-step28-reservation-draft-2a327798`; restore and adapt it only for the following reservation item after this prerequisite commits. No cold checkout/build or broader scalar architecture redesign.
  - Steps and boundaries: use Rust `opforge-core/src/expr.rs` and the live VM evaluator as operator authority. Preserve both little-endian words of existing i64 literal bytecode; make the eight existing logical stack slots pairs of longs; implement full-width unary/binary/ternary behavior and preserve legacy D3-low/status callers with an explicit high-result accessor. Rust symbols, lexical resolver values and current addresses originate u32 and must zero-extend; do not allocate high-word label/snapshot/statement tables. Upgrade bridge numeric literal parsing/emission to Rust's actual accepted forms and width, and carry the result high word through the bridge/service. Audit all extension field lifetimes and legacy lengths before using consumed input fields as result high/width metadata; do not assume a stored length authorizes an overlapping live field. Expose one authoritative typed scalar result so the reservation consumer can require high=0 without sign-text or low-bit heuristics. Remove temporary duplicate RES literal/sign paths when restoring its draft in the following item. Retain the full original reservation and Phase A scope.
  - Evidence and proof: live Rust boundary pairs distinguish `$80000000+0`, `-2147483648`, `$ffffffff+1`, and `($ffffffff+1)-1`. Direct runtime tests cover both opcode versions, high literal words, carry/borrow, sign/zero extension, signed comparisons/division/modulo, bit/logical operations, ternary, powers, shifts and malformed programs/stack limits. Match Rust's wrapping i64 add/subtract/multiply/power and actual shift-count rules; establish minimum-i64 unary/division behavior with a live oracle and record failure-domain treatment explicitly rather than inventing arithmetic. Fresh native proof must return exact same-program live Rust high/low results with explicit zero guest exits; negative cases require fresh completion, explicit nonzero exits and named diagnostics. Include bridge literal/symbol and service typed-result/short-client compatibility proof. A passing low32 or small-literal subset is insufficient.
  - Before/after and stop/go: correctness prerequisite, no promised runtime gain; report step and cumulative gains as unmeasured unless matched timing exists. Keep fixed stack/result growth measured, avoid per-statement growth and formatted-text round trips for typed consumers. Stop after the shared scalar authority is proven; reservation placement/rendering and later footer repairs remain separate.
  - Gate tier: one end-to-end scalar value authority invariant, with direct runtime inner-loop proof before full service confirmation.
  - Required focused gates: live Rust scalar oracle (A), ABI/ownership and wire contracts (B), exact fresh native runtime/bridge/service proof (D), native formatter, architecture, inventory and capacity.
  - Full quality gates: mandatory non-LSP Rust, staged native and workflow/plan validators; required affected expression, scalar operand, macro/conditional/repetition/forward-reference/module-visibility regressions; broad frozen corpus and all current nonterminal native groups remain at A-close.
  - Plan-compliance review evidence: PASS — `step28_scalar_review` independently verified operator authority, extension/result lifetimes, ABI/zero-extension, proof boundaries, all seven affected native regressions and required gates. The reviewer also approved the exact-path/exact-call functional-output checker exception and these mechanical completion edits for this atomic commit.
  - Commit outcome: one focused native signed-scalar authority commit before restoring/completing the reservation item.
  - Definition of done: the native shared result preserves the Rust signed i64 value through evaluated operators and typed service transport, all required proofs/gates/review pass, and reservation plus every other Phase A obligation remains open.

- [x] Step 29 · Item A-listing-reservation — Render reservation status, address and size in source rows
  ...
  - Expected files: native listing renderer, reservation driver and engine semantic transport/accessors, focused reservation listing tests, measured budgets, slice metadata and evidence.
  - Dependencies/worktree: committed A-cpu-execution-contract and A-scalar-i64; restore the preserved reservation draft in the warm worktree and use its explicitly identified candidate package.
  - Steps and boundaries: Match Rust `ListingWriter::write_line` DirDs behavior using authoritative retained reservation semantics, resolved start address and reservation byte count. Render the address and plus-prefixed size even when no output bytes are emitted. Do not infer reservation from every zero-byte statement, reparse source, change BSS/image presence or placement semantics, or touch later symbol/generated-output footer behavior. The driver currently mistakes operand-2 count for byte extent. Resolve unit byte=1, long=4, word from the selected CPU execution contract, or a nonnegative numeric expression; reject zero units, negative values, multiplication overflow and a nonempty span exceeding the selected maximum program address. Match Rust `update_addresses` as well: the next section program counter after the reservation must not exceed that maximum or overflow, even when the last occupied byte fits. Use the same checked extent for placement/PC advancement and listing retention. Then retain that exact extent without new storage: a dedicated engine setter stores it in the existing statement output-count slot for RES, the ordinary emitted-byte getter recognizes the retained directive kind and still returns zero for RES, and a dedicated reservation getter exposes the raw extent only for RES. Audit every existing output-count reader/writer and preserve ordinary emitted-byte behavior through its getter; raw-table interpretation remains engine-owned. Preserve field-width/capacity contracts and reject truncation. Do not infer size from successor addresses or allocate a new per-statement table.
  - Evidence and proof: A complete label-free fixture emits one code byte at $8000 followed by BSS reservation of four bytes; live Rust prints `8001` and `+0004`, one-byte total memory and generated output, and no symbols. Require exact whole live-Rust listing and companion BIN under fresh zero-exit native proof. Focused native proof must cover byte/word/long and a nonstandard numeric unit, zero count, and negative/overflow/span-bound failures with exact exit/diagnostic scope. Host semantic contracts distinguish reservation from zero-byte directives and establish unit scaling and capacity/failure behavior, and ensure no reserved BSS bytes become emitted image bytes. Rerun unchanged B09 once for all eleven artifact dispositions. Any newly exposed footer discrepancy remains open.
  - Before/after and stop/go: correctness qualification only; step and cumulative runtime gains remain unmeasured absent matched evidence. Stop after this reservation rendering invariant is proven; later footer repairs require their own reviewed item before A-close.
  - Gate tier: one reservation listing status/auxiliary-value invariant.
  - Required focused gates: live Rust oracle and native semantic ownership contracts (Levels A/B), complete fresh native exact listing/BIN (Level D), B09 disposition, native formatter, architecture, inventory and measured budgets.
  - Full quality gates: mandatory non-LSP Rust, staged native and workflow/plan validators; broad qualification remains at A-close.
  - Completion evidence (2026-09-08): Step29 results ledger records nine mixed positive and nine negative fresh native cases, plus two final full-u32 cases and a separate complete-fixture confirmation. The aggregate with two failed BSS-only cases remains explicitly failed; unchanged per-case proof reuse is independently approved. B09 remains 10/11 exact, with its first mismatch moved from reservation row19/byte1260 to symbol-footer byte2600. Final non-LSP Rust, staged native, workflow, plan, capacity and inventory gates pass. Runtime gains remain unmeasured; no per-statement storage is added.
  - Plan-compliance review evidence: PASS — step28_scalar_review independently verified the final invariant and scope, approved retained per-case evidence and the separate BSS-origin/empty-image debts, and authorized these mechanical completion edits once all final gates passed.
  - Commit outcome: one focused reservation listing correction commit before the next causal repair or A-close.
  - Definition of done: focused whole artifacts equal Rust, reservation placement uses checked unit-scaled extents without emitting BSS bytes, and all required gates/review pass; all remaining B09 and Phase A debt stays explicit.

- [x] Step 30 · Item A-bss-image-origin — Prevent BSS origins from creating artificial flat-image gaps
  - Source requirement or finding IDs: SR-PAR, SR-MEAS; Step29 failed mixed full-u32 cases; existing overlapping-origin slice invariant that BSS does not establish the flat image origin.
  - Expected files: `native/motorola68000/amigaos/opasm/opasm_assembly_driver.asm`, `native/motorola68000/amigaos/opasm/opasm_layout.asm`, a focused native BSS-origin test module/registration, slice metadata, measured capacity/inventory snapshots and results.
  - Dependencies/worktree: Step29 commit `6f0f82cca5d7ab849f4f4fe4c549a16a0708613f`; use the warm native worktree and integrate the focused commit without touching the unrelated CLI-directory rename.
  - Steps and boundaries: In the existing origin helper, consult layout-owned active BSS state before recording/minimizing the flat-image origin. BSS `.org` still changes its current PC, but must not set image-base-seen or change the flat-image base. In layout-owned statement image routing, discard BSS output requests, including nonzero alignment padding, while preserving all non-BSS main/mapped/logical/unplaced routing. Rust permits BSS alignment but suppresses its pad bytes. Preserve each unplaced section's retained pass-one start across retries and final emission so image-base finalization cannot change its size or labels; placed sections retain their supplied base. This includes the exact mapped logical-section control whose same convergence failure was reproduced at the Step30 base commit. Route-2 packing/reachability and broader logical-section semantics remain unchanged. Preserve unsectioned and concrete-section origin handling, placement and reservation sizing; no new state/table or CLI contract. Use Rust section BSS non-emission and ImageStore output range as the reference. Do not mix in symbol-footer, ORG-row rendering, zero-image output, or other logical-section changes.
  - Evidence and proof: Complete one-byte CODE placed at $8000 plus BSS `.org 0` with small and full-u32 reservation counts, covering both unplaced and placed BSS below CODE, must produce the exact one-byte live Rust BIN under fresh zero-exit native proof. Include placed BSS below CODE with a nonzero `.align` pad and exact one-byte BIN, so a wrapped image cursor cannot hide behind zero-size reservations. Retain the prior full listing/BIN failure as history; BIN qualification does not claim ORG-row or generated-footer parity. Add the established complete mapped logical-section exact-BIN control and one complete unsectioned forward/backward-origin BIN control to preserve ordinary gap/overwrite behavior. Every actual test owns its complete source, command, live in-memory Rust oracle, fresh guest protocol and ephemeral outputs; attempt every case even after failure.
  - Before/after and stop/go: Source analysis of the recorded failing placement predicts removal of $8000 artificial gap bytes before its one code byte. Verify exact final bytes; do not label the previous unverified BIN length or a failed-run duration as a measured speedup. Record actual artifact length, fixed-state/executable growth where measured, step gain and cumulative gain. Stop after BSS image-origin handling is proven; all listing and other Phase A debts remain open.
  - Gate tier: one image-origin and independent unplaced-section convergence invariant, with a representative complete native case first and the remaining focused cases only after it passes.
  - Required focused gates: actual Rust BIN oracles, native ownership/status contract, complete fresh native BIN proofs, native formatter, architecture, inventory and measured source capacities.
  - Full quality gates: final non-LSP Rust quality gate, staged-native and workflow/plan validators, then independent compliance. Run the full Rust gate after focused native behavior passes; retain live handles and do not restart for observation expiry. Broad native/corpus qualification remains at A-close.
  - Plan-compliance review evidence: independent PASS on BSS image exclusion and unplaced-section convergence scope, ordinary-origin control, no new storage, proof limits and exact final receipts before commit.
  - Commit outcome: one focused BSS-origin correction commit before the next repair or A-close.
  - Definition of done: BSS origins and alignment cannot expose flat-image bytes or gaps, the complete BSS/control BINs equal live Rust with required fresh proof, and all required gates/review pass; no broader listing or Phase A completion is claimed.

  - Completion evidence (2026-09-08): six complete final-code native BIN cases pass, including the mapped logical control whose same convergence failure was reproduced at exact prior commit `a893636e`. BSS origins/padding contribute no image bytes; independent unplaced sections retain their starts across final emission. Final non-LSP Rust gate passes once (262.655s), along with current capacities, staged-native, workflow and plan validators. No runtime storage is added. The exact one-byte BSS/code output is proven; 32,768 avoided artificial gap bytes are source-derived, and step/total runtime gains remain unmeasured. Results: `documentation/performance/results/opforge-step30-bss-image-origin-2026-09-08.md/.json`. Broader section-start, listing, zero-image and other Phase A debts remain open.

- [x] Step 31 · Item A-map-key-cache — Cache lowercase map sort keys once per symbol
  - Source requirement or finding IDs: SR-EARLY, SR-RF, SR-MEAS, SR-PAR; 2026-09-08 `/private/tmp/opforge-step31-map-key-baseline` actual-function baseline.
  - Expected files: `crates/opforge-vm/src/output_artifacts.rs`, focused VM tests, this plan and Step31 result/receipt artifacts.
  - Dependencies/worktree: committed Step30 at `24efe391`; the active worktree `AGENTS.md` remains binding. Native code and the unresolved native listing-symbol footer are outside this Rust-only item.
  - Steps and boundaries: In `build_mapfile_text`, replace comparison-time `to_ascii_lowercase` allocation with one cached lowercase key per symbol, using `sort_by_cached_key` and preserving the current stable lowercase ordering and valid `SymbolTable` domain. Preserve region, section, visibility, address and text formatting exactly. Use the four retained baseline outputs for N=36 and N=4096 in All/Public modes as actual-function byte-for-byte oracles; no evidence filename selects expected behavior. Do not change native map/listing code or claim a native runtime gain.
  - Before/after and stop/go: Re-run the same ordinary-allocator actual-function harness with prebuilt symbol table/directive/maps, five samples of ten iterations after warm-up, plus a separate one-build allocation counter. Retain representative N=36 and high-label N=4096 raw samples, medians/ranges, allocation/reallocation counts, compiler identity, retained static executable identities, exact source identities and exact output hashes. Accept only if all four outputs are byte-identical and allocation events and median time improve materially beyond observed variation; otherwise revert the candidate and record no-go. Report the temporary peak memory cost of one lowercase key per symbol; do not infer whole-CLI or native savings from this microbenchmark.
  - Gate tier: focused sub-item.
  - Required focused gates: actual-output behavior tests covering mixed-case, punctuation and long canonical names plus All/Public/None and empty tables over valid `SymbolTable` entries; folded-key ties are unreachable under the table uniqueness contract, while stable sorting remains the selected API contract; exact four-artifact baseline/candidate comparison; workflow and plan validators; common execution/gate policy applies.
  - Full quality gates: one `scripts/workflow/run_rust_quality_gate.sh --defer-lsp` run, plus workflow and independent plan compliance before the focused implementation commit. No native runtime gate is required because native production is unchanged; broad native qualification remains at A-close.
  - Plan-compliance review evidence: independent PASS must cite the plan insertion/renumbering, exact production/test diff, baseline/candidate receipts, artifact identity, benefit/peak-memory limits and required gate results.
  - Commit outcome: one focused `perf(vm): cache map symbol sort keys` implementation commit including this justified plan amendment after all required gates and review pass.
  - Definition of done: `build_mapfile_text` produces identical map text, the measured actual-function allocation/time benefit satisfies the stop/go rule, required Rust/workflow/review gates pass, and native footer plus every A-close debt remain open.

  - Completion evidence (2026-09-08): exact four map outputs retained; actual-function opt-level=1 timing improves 63.17–77.51% and allocation events fall 76.19–88.62% across N36/N4096 All/Public fixtures. The single non-LSP Rust gate, focused output tests, workflow gate and independent step28_scalar_review compliance pass. Fixed-order timing bias and temporary cached-key storage are explicit in `documentation/performance/results/opforge-step31-map-key-cache-2026-09-08.md/.json`. This does not establish release, whole-CLI or native runtime gains. Native listing/footer and all remaining Phase A debt stay open.

- [x] Step 32 · Item A-listing-symbol-footer — Render canonical native listing labels
  - Source requirement or finding IDs: SR-PAR, SR-EARLY; Step29 B09 first symbol-footer mismatch and final placed-value source audit.
  - Expected files: `native/motorola68000/amigaos/opasm/opasm_output_artifacts.asm`, CLI `output.asm`, `source_artifacts.asm` and new `symbol_metadata.asm`; focused listing/map tests and coordinator registration; measured source-capacity expectations; this plan, native slice metadata and result receipts.
  - Dependencies/worktree: Step31 committed at `600ce722`; active `AGENTS.md` remains binding. Preserve the integration worktree's CLI directory rename and all unrelated user content; carry authorized changes through the rename only after exact baseline/path verification.
  - Steps and boundaries: Match Rust `listing.rs::write_symbol_table` for immutable root labels: emit the engine's canonical NUL-terminated name exactly once without truncation, pad only to minimum width15, use uppercase lexical order then raw-name and original-index ties, read `opasmEngineGetLabelPlacedValueV1`, format full-u32 values at 4/6/8 digits with minimum width8, and use recorded root export visibility. Use a bounded no-state lexical-successor scan in the artifact renderer; do not alter map sorting policy or add a persistent sort buffer. Extract the existing CLI root visibility resolver and its name comparison into a small CLI-owned module, preserving its behavior. Supply it via an explicitly documented callback entry; retain the existing listing entry as a private-visibility fallback. Generic opasm must not import CLI or guess visibility from spelling. Document register/CCR and stack contracts; add no ad-hoc instrumentation.
  - Canonical source-label amendment: The second representative completes with zero guest exit and exact BIN but fails whole-listing equality solely at retained leading-label colons. Match Rust `listing.rs::normalize_leading_label_colon` in a bounded output-only appender: recognize leading ASCII identifiers after spaces/tabs (start: letters or underscore; continuation: letters, digits, underscore, dot or dollar), remove only their immediate colon, add a separator only before a nonempty code remainder lacking space/tab, and preserve indentation, comments, quoted operands and every other source byte. A comment immediately after the colon receives no new space. Do not mutate source records or parsing; do not drop colons from the proving fixture to bypass this divergence. This is part of canonical label display in the same listing artifact, adds no persistent state and retains the original failed evidence.
  - Scope limits: This item corrects canonical source-label spelling and the named root immutable-symbol rendering boundary. Mutable `var` kind and non-root public visibility need authoritative metadata and their own proof before A-close. Generated image/present-byte footer and ORG-row debts remain open; a moved B09 failure is not their closure.
  - Before/after and stop/go: Correctness prerequisite, no runtime speedup promised. Record exact artifact identity, source/storage costs and any added ordering work (successor scanning is O(N^2)); report step and cumulative gains separately. A sorted-index performance replacement needs measured motivation and its own reviewed item, not an unmeasured map rewrite here.
  - Gate tier: focused native output boundary; representative complete case first, then discriminating placed-address and unchanged-map controls.
  - Required focused gates: Level A live Rust full BIN/listing oracles with canonical long names, mixed-case/punctuation order, public/private labels and final 4/6/8-digit addresses. Use PC-backed immutable labels in placed BSS sections at 4-, 6- and 8-digit addresses, with exactly one emitted CODE byte, so final placement and BSS non-emission are observable rather than using constants whose stored/final values coincide. Include alphabetic and underscore name boundaries that distinguish listing ASCII-UPPER order from map ASCII-lower order, while the unchanged map keeps its own lower-order oracle; Keep both a no-space `label:.res` row and a spaced column-one label row with a comment colon in the complete native fixture; leading indentation is exercised through the actual Rust ListingWriter boundary because the Rust CLI rejects the attempted indented declaration. Level A listing normalization controls for spaced/unspaced colons, indentation, label-only/comment tails and untouched directive/comment/quoted-operand colons; Level B source/ABI review; Level D fresh exact BIN plus full listing for complete semantic fixtures, plus existing exact all/public map control. Use only actual source/command/package and in-memory same-case Rust oracles, fresh start/completion, explicit zero exit, ephemeral artifacts and attempt-all behavior. Re-run unchanged B09 after the focused proof and record every remaining mismatch without suppressing artifacts or extending its timeout.
  - Full quality gates: native assembly/formatter, source capacity, staged native gate with slice metadata, one non-LSP Rust quality gate after focused behavior passes, workflow/plan validators and independent plan compliance. Retain live gate handles; broad native/corpus qualification stays at A-close.
  - Plan-compliance review evidence: independent PASS must inspect exact rendering/source-colon normalization/callback/metadata extraction, preserved map semantics, proof scope and all required receipts; validate rename-aware integration does not alter unrelated content.
  - Commit outcome: one focused listing-symbol correction commit before the next debt repair; safe local integration only, no push.
  - Definition of done: complete focused native artifacts equal their actual live Rust oracles, map regression and required gates pass, callback/frame safety is reviewed, and all wider Phase A debts remain explicit.
  - Completion evidence (2026-09-08; focused commit and safe integration pending): actual live Rust and fresh native representative/placed-address BIN plus whole-listing artifacts match exactly with explicit zero guest exits; the unchanged all/public map case also passes exact fresh Level D proof. B09 checks all 11 artifacts under the current package: ten match and only its generated/ORG listing footer remains open. The first leading-underscore fixture failure and the second two-colon listing localization remain retained. Native assembly/formatter, final source capacity, focused host, staged-native and workflow gates pass. The first full non-LSP gate retained three stale test-contract failures; their V2 callback, 101-file staging and bounded-normalizer assertions were corrected, focused checks pass, and the single final non-LSP gate passes. No Step32 runtime gain is claimed; all Step33 dependencies and recorded debts remain open.

- [x] Step 33 · Item A-leading-underscore-label — Accept leading-underscore source labels in native preprocessing
  - Source requirement or finding IDs: SR-PAR, SR-EARLY; retained Step32 243-byte source SHA-256 `e0418c29e4f6d1b387760b2dc8d00a70f4e35f7fa7b47bbd69665f47adb46e63` and `OPC-NCLI038`/`OPC-NCLI039`/`OPC-NCLI010` failure.
  - Expected files: `native/motorola68000/amigaos/opforge-cli/preprocessor_invocation.asm`; existing `crates/opforge-asm/src/tests/native_listing_symbols.rs` with its unchanged coordinator registration; measured source-capacity expectations; this plan, `documentation/plans/slices/native-performance-leading-underscore-label-v1.toml` and result receipts.
  - Dependencies/worktree: Step32 committed at `610db37b`; active `AGENTS.md`, `agents/rules/native-rust-parity-porting.md`, `agents/rules/native-parity-failure-triage.md`, `agents/rules/native-68000.md` and `agents/rules/fs-uae.md` remain binding. Preserve the integration worktree CLI directory rename and unrelated user content through the reviewed rename-aware transaction.
  - Steps and boundaries: Match Rust `opforge-core/src/text_utils.rs::is_ident_start` and macro `parse_label`/`parse_macro_invocation` by accepting `_` as the first byte in `preprocessor_invocation.asm::captureInvocationLabel`. Add only the architecture-neutral underscore branch before the existing ASCII letter range; preserve continuation scanning, colon consumption, capacity, macro-invocation and diagnostics behavior. Add no CPU/family logic, new storage, parser rewrite or unrelated compact-expression grammar change.
  - Scope limits: This closes only the leading-underscore first-byte rejection for ordinary/macro preprocessing labels. Internal punctuation continuation, listing source normalization and root immutable footer rendering remain the completed Step32 behavior. Mutable/non-root metadata, compact symbol/operator grammar, generated/ORG footer, zero-image emission, section-start and all other A-close debt remain open.
  - Before/after and stop/go: Correctness repair only; no runtime gain is promised. The retained failed source and receipt remain historical localization evidence. Keep only if the exact same 243-byte source produces byte-identical live Rust BIN/listing artifacts in a fresh guest and existing macro invocation remains exact; otherwise revert or record the first new divergence without weakening the source.
  - Gate tier: focused native frontend boundary.
  - Required focused gates: Level A actual live Rust BIN/listing oracle from the exact retained 243-byte source and package; Level B source/ABI review of the single first-byte branch with no implementation-mirroring simulator; Level D fresh exact BIN plus whole listing for that same source with fresh start/completion, explicit zero guest exit and ephemeral artifacts; unchanged `native_macro_invocation_fixture_fs_uae` Level D control. Preserve attempt-all behavior and retain all failed receipts.
  - Full quality gates: native assembly/formatter, measured source capacity, one non-LSP Rust quality gate after focused native proof, staged-native with `native-performance-leading-underscore-label-v1.toml`, workflow/plan validators and independent plan compliance. Broad native/corpus qualification remains at A-close.
  - Plan-compliance review evidence: independent PASS must inspect the exact one-branch production diff, retained source/package identity, complete same-case oracle, macro control, all required receipts, unchanged storage/runtime claims and preserved A-close debt.
  - Commit outcome: one focused `fix(native): accept leading underscore labels` commit before A-close; safe local integration only, no push.
  - Definition of done: the retained source passes exact fresh native parity, the macro control and required gates pass, final independent compliance approves the exact diff, and every wider Phase A debt remains explicit.
  - Completion evidence (2026-09-08; focused commit and safe integration pending): the exact retained 243-byte source SHA-256 `e0418c29e4f6d1b387760b2dc8d00a70f4e35f7fa7b47bbd69665f47adb46e63` now passes its live Rust host oracle and fresh native exact BIN/whole-listing proof with explicit guest exit 0. The unchanged macro-invocation Level D control independently passes with exit 0. Native assembly/formatter, measured capacity and the single full non-LSP Rust gate pass. No storage or runtime gain is claimed; all other A-close debt remains open.

- [x] Step 34 · Item A-throughput-refresh — Refresh the current statement and integrated throughput boundaries
  - Source requirement or finding IDs: SR-EARLY, SR-MEAS, SR-PAR; user prioritization of the cheapest largest gains and faster following performance work. Latest complete-attempt B03 evidence is Step13's six 120-second timeouts; latest B10 is Step19's profile-off byte/buffered 120-second timeouts. Step18's controlled one-visit abort is Level E only. No later B03/B10 completion was found through Step33.
  - Expected files: this plan and quality-gate sidecar; `documentation/performance/results/opforge-step34-throughput-refresh-2026-09-08.md` and adjacent JSON. No production, fixture, test-runner or oracle changes.
  - Dependencies/worktree: Step33 committed at `40a81b4c`; active `AGENTS.md` and native parity/failure-triage/FS-UAE rule packs remain binding. Use the qualified warm implementation worktree and preserve the integration worktree's pending CLI rename.
  - Steps and boundaries: Attempt exactly one current profile-off B03 and one B10 through the existing `external_fs_uae_native_production_corpus_parity` runner, with the actual current locked release Rust oracle and shipped package. Run each independently and attempt B10 even if B03 fails. Keep frozen source/argv/outputs, the 120000ms post-start timeout and 250ms polling. Explicitly select longword session clear, live statement clear, buffered module/source reads, and no diagnostic/abort/live-capture mode. Do not restart a live handle or repeat a failed case without a new reviewed discriminator.
  - Input identity: B03 case SHA-256 `be04f9426a0ffb55278cdb05d1d604f5895e85a2cee0b6894ac43c9df1359acf` has 3,083 source bytes/257 lines and requires `output.bin`; B10 case SHA-256 `0e8c45ef53ca91471524bbb030720fc4bb4e7601fa30f5b12b0e94d02ec9484c` has 23,770 bytes/1,358 lines across its files and requires BIN, integrated PRG and map. Derive oracles only from the actual case, never these display identities or old result files. Record the current corpus/package/source/native-image identities and exact environment.
  - Before/after and stop/go: This is a bounded qualification refresh, not an optimization or a matched historical timing comparison. Only completed exact-artifact, empty-stream, zero-exit cases supply host-observed START-to-DONE timing; retain 250ms polling uncertainty separately from whole-test cost. A timeout remains incomplete and never supplies a speedup ratio. Report step and cumulative gains separately; total end-to-end gain remains unmeasured without a matched series. Use the first current failure or completed timing to prioritize one next numbered repair/measurement item; preserve the pending listing and all wider debts.
  - Gate tier: bounded current-state observation; at most two native attempts plus current bootstrap/oracle build cost.
  - Required focused gates: existing fail-closed Level D actual-source/package/command proof, fresh challenge/start/completion, explicit zero guest exit, exact complete artifacts, empty stdout/stderr and ephemeral cleanup. Classify a timeout or failed artifact as no parity proof; attempt both cases and retain the full failed receipt. Check receipt hashes and source/case identities independently.
  - Full quality gates: workflow/plan/artifact validators and independent plan compliance before the focused results commit. No Rust/native production or test code changes occur, so reuse Step33's qualified code state; full non-LSP Rust, all nonterminal groups and full corpus qualification remain mandatory at A-close.
  - Plan-compliance review evidence: independent PASS must verify both attempted terminal results, all declared artifacts and proof statuses, unchanged budgets/inputs/modes, exact current identities, honest timing limits and the next-action decision.
  - Commit outcome: one focused `docs(perf): refresh native throughput blockers` commit with safe local integration and no push before the next item.
  - Definition of done: both cases have authoritative terminal receipts and honest independent dispositions, the current next-action decision is recorded, workflow/compliance passes and the focused commit is complete. This does not close either B10's five-completed-run censored-baseline requirement or Phase A.
  - Completion evidence (2026-09-08; focused result commit pending): exactly one current profile-off B03 attempt and one current profile-off B10 attempt used the unchanged 120000ms post-start boundary, 250ms polling, longword/live/buffered modes, shipped package and actual live Rust case oracle. Both produced a fresh `CORPUS_START`, then timed out without `CORPUS_DONE`, `CORPUS_RESULT`, explicit guest exit or artifact comparison; both remain incomplete and supply no runtime or parity result. The result ledger retains their exact commands, environments, identities and terminal receipts. A bounded one-entry successful CTBL lookup memo experiment is selected ahead of listing repair from static package/source work evidence only; it is not attributed as either timeout cause and has no measured gain. The B10 five-completed-candidate rule, every wider Phase A debt and A-close remain unchanged.

- [x] Step 35 · Item A-compact-zero-memo — Reuse the last successful zero-operand compact program lookup
  - Source requirement or finding IDs: SR-EARLY, SR-MEAS, SR-PAR; Step34 current B03/B10 timeouts and static CTBL work audit. Both remain incomplete, and the proposed mechanism is not established as their cause. Active `AGENTS.md`, plan-authoring skill and native parity/68000/FS-UAE rules remain binding.
  - Expected files: `native/motorola68000/amigaos/tkpkg/tkpkg_compact_table.asm`, `tkpkg_buffers.asm`, `tkpkg_pipeline.asm`; a focused `test-harnesses/tkpkg/tkpkg_compact_memo_harness.asm`; `crates/opforge-asm/src/tests/native_compact_memo.rs`, existing test-module/FS-UAE harness and production-corpus wiring, measured source-capacity expectations, this plan/sidecar, one slice manifest and result MD/JSON. Adjust only affected inventory/formatter metadata when required.
  - Dependencies/worktree: A-throughput-refresh committed and safely integrated; use the warm worktree, preserve the integration worktree's pending CLI rename, and do not push. This item precedes listing repair and A-close; all recorded debt remains binding.
  - Rust reference boundary: `crates/opforge-package/src/package/codec.rs` decodes actual package CTBL programs; actual Rust model selection/CLI output remains authoritative. Native boundary: `tkpkg_compact_table.asm::findFixedProgramFromRequestV1`, package-derived buffers and public `tkpkgPipelineSetActiveV1`. This reuses a selected program, never encoded output or CPU-specific semantics.
  - Steps and boundaries: Add one successful-result memo eligible only when the validated request has zero shape counts and the existing code chooses its canonical mode. Key exact mnemonic bytes plus length, bounded to 32 bytes; case variants safely miss and use the unchanged casefold lookup. Shaped, oversized, missing and malformed requests retain the original lookup behavior and do not publish or evict the memo. This allows shaped instructions between repeated zero-operand requests without thrashing the entry. Store only a package-relative program offset and nonzero length; validate the complete request shape/window/mnemonic before probing, check offset-plus-length overflow and package length as well as the current CTBL extent on a hit, invalidate a corrupt memo and fall through the unchanged cold parser, publish validity last, and preserve caller registers/status/stack behavior. Keep all memo storage inside the existing package-state clear range and clear validity at public pipeline-selection entry before every fallible operation, covering successful/failed selection and same-address package reload. Preserve the existing service-init contract. No package mutation between load and use is introduced or supported.
  - Bounded cost: fixed mnemonic storage plus offset/length/validity metadata, no allocation, sorting, unbounded table, per-opcode I/O or new instrumentation. Record exact BSS, stack, source and native-image changes and verify the public 16-bit control-block window remains valid. An assembly-time `OPFORGE_COMPACT_ZERO_UNCACHED_REFERENCE` bypass and explicit corpus selector may compare the same source with memo hit/store disabled; reference and candidate share storage/clear costs and all other modes. Retain this distinction in results; do not call the instrument-free control the exact historical binary.
  - Cheap screen before expanded proof: implement production first, inspect source/ABI/lifecycle and assemble, then run one completed uncached-reference B01 followed by one candidate B01 through the unchanged fresh live-Rust corpus proof. Use frozen input/package/argv, profile off, longword/live/buffered modes, 120000ms post-start timeout and 250ms polling. Continue only if both are exact zero-exit completions and candidate START-to-DONE time improves by more than max(5%, the 500ms paired polling uncertainty divided by control time). This first pair is screening only, not retention evidence. If either fails or no clear gain appears, stop expansion, retain failure/no-go receipts and revert the candidate unless one bounded, independently reviewed first-divergence correction is justified; never blindly repeat a failed run.
  - Retention proof after positive screen: use a dedicated native harness patterned after the existing CPEX loader/selection harness to emit actual returned program bytes/status to a binary artifact, with live Rust-decoded actual package expectations. Cover repeat zero key, changed zero mnemonic, shaped explicit-mode bypass with distinct expected program, case variant and over-cap fallback, successful pipeline selection with the same mnemonic resolving to distinct program bytes, failed reselection followed by an old-context request, successful same-address reload with a second valid package yielding a distinct result, and failed reload/malformed table. Use genuine API transitions and package fixtures; do not mutate loaded package bytes merely to demonstrate a hit. Level A establishes actual package and selection/output expectations; Level B independently reviews eligibility, bounds, invalidation, ABI and storage; Level D proves complete fresh guest outcomes and bytes. Reuse existing malformed CTBL/version/opcode controls where they directly apply. A small complete mixed-instruction CLI fixture must match the same-case live Rust output. Attempt all planned cases even after an individual failure.
  - Before/after and stop/go: after the screen, add two B01 pairs in candidate/control then control/candidate order (three total per mode); report raw START-to-DONE samples, medians/ranges, 250ms polling uncertainty, complete artifact identities, stable per-mode native-image digests and whole-test cost separately. Require zero mismatches and median benefit greater than max(5%, observed range/noise, paired polling uncertainty); otherwise revert/defer. Only after focused proof and benefit pass, attempt one current B03 and one B10 independently at the unchanged 120000ms limit; timeouts remain debt and never provide speedup ratios. Existing B10 five-completed-candidate or matched non-regression closure remains at A-close. Report step and cumulative scoped gains separately; total end-to-end gain remains unmeasured without a matched initial/current series.
  - Gate tier: screened native performance experiment with mandatory retention checks; wider certainty and all nonterminal groups remain at A-close.
  - Required focused gates: source/ABI/lifecycle review; both reference/candidate native assembly; focused Level A/B/D package/selection/mixed-CLI proof, malformed controls, measured source and ABI capacity, native formatter/staged/architecture/inventory and exact receipt checks. Fresh challenge/start/completion, explicit guest exit, actual in-memory Rust oracle and ephemeral cleanup remain fail-closed for every case; actual process-negative CLI cases require their exact nonzero guest exit and diagnostic scope; harness-contained negative API rows require exact returned status/diagnostic bytes as data and explicit guest exit zero for successful proof.
  - Full quality gates: one final non-LSP `scripts/workflow/run_rust_quality_gate.sh` after focused checks for the Rust runner/test changes; workflow/artifact/plan validators, staged native slice and independent plan compliance before commit. Do not restart live gates or rerun a broad gate for receipt-only edits. Full corpus/nonterminal native qualification stays at A-close.
  - Plan-compliance review evidence: independent PASS must inspect the production eligibility/key/result/invalidation boundaries, all accepted and rejected attempts, exact proof levels and inputs, measured gain/noise and memory cost, required gate receipts and preserved failure debt. Plan-quality PASS is required before production execution.
  - Commit outcome: one focused `perf(native): memoize zero-operand compact lookups` if retained, or a focused documented no-go with candidate production reverted; safe local integration only. Commit this item before the next begins.
  - Definition of done: the candidate has a supported retain/revert decision, required proof/gates and final independent compliance pass, result evidence and focused commit are complete, and all wider Phase A debt remains open until independently qualified.
  - Completion evidence (2026-09-08; focused commit requires final gated compliance): retain the zero-shape memo after three alternating B01 pairs, each with fresh exact artifacts and guest exit zero. Median START-to-DONE improves from 36.996669833s to 34.428058125s (6.9428%); ranges and paired polling uncertainty remain below the 5% retention threshold. B03 now completes in 37.735005375s with exact live Rust BIN parity on the same memo image; its Step34 timeout stays historical evidence, not a denominator for a speedup ratio. The independent B10 candidate again times out at the unchanged 120000ms post-start boundary without completion, explicit guest exit or artifacts, and remains open under the existing five-completed-candidate/matched-control A-close rule. The 24-command lifecycle batch, mixed CLI and unchanged malformed version/opcode controls pass fresh native proof. Source/ABI review, assembled layout (40-byte record plus 3-byte alignment; 8 extra stack bytes; package CB offset 12812), six capacity checks, formatter and the single final non-LSP Rust gate pass. Final staged/workflow and independent compliance receipts govern the focused commit. Current B03 timeout debt is superseded by this exact completion; every other Phase A debt and LSP-last remain unchanged. Total Phase A runtime gain remains unmeasured.

- [x] Step 36 · Item A-compact-shaped-memo — Test bounded reuse of successful shaped compact lookups
  - Source requirement or finding IDs: SR-EARLY, SR-MEAS, SR-PAR; Step35 B03 exact completion at 37.735005375s, retained B10 timeout, and the bounded Step35 CTBL ownership/result contract. Repeated shaped-key workload evidence remains a static audit until measured; no timeout cause or gain is established.
  - Expected files: `native/motorola68000/amigaos/tkpkg/tkpkg_compact_table.asm`, `tkpkg_buffers.asm`, and `tkpkg_pipeline.asm`; existing compact-memo lifecycle/Rust proof, production-corpus selector, capacity expectations, this plan/sidecar, a focused slice manifest, results and affected inventory metadata only as required. Reuse the Step35 harness and runner surfaces where they remain sufficient; do not add a generic cache framework.
  - Dependencies/worktree: Step35 committed at `132e31b3`; active `AGENTS.md`, plan-authoring and native parity/68000/FS-UAE rules remain binding. This item precedes A-close; every wider Phase A and B10 closure requirement remains open.
  - Rust/native authority: actual package CTBL decoding and `RuntimeModelCore::encode_candidates` define returned programs and output. Native ownership remains `findFixedProgramFromRequestV1` plus package-state storage and pipeline-selection invalidation. CPU, mnemonic meaning, operand shapes, opcodes and output semantics remain package-owned.
  - Steps and boundaries: retain the proven one-entry zero-shape memo unchanged and add at most two successful shaped entries. Each key contains exact mnemonic bytes up to 32 and exact canonical mode bytes up to 64; shaped requests outside either bound and missing/malformed requests safely use the unchanged cold path. Exact case variants miss a differently cased key, then may publish their own exact key only after a successful eligible cold lookup. Validate the complete request and current package/CTBL window before probing. Store only bounded package-relative program offset/nonzero length, revalidate offset-plus-length overflow and both package/CTBL extents on hits, invalidate corrupt entries and cold-fallback, publish validity last, and invalidate all shaped entries on package load/reload and every successful or failed pipeline-selection attempt. Successful eligible cold stores use deterministic round-robin replacement; zero-shape traffic, misses, errors and over-cap requests neither consume nor evict shaped entries. Do not cache output bytes, errors, ambiguity, misses, CPU-specific identities or semantic results.
  - Bounded cost: at most 220 additional BSS bytes plus measured code and stack; exact assembled alignment, package-state clear growth, 16-bit control-block offsets, source graph and public/import budgets must be recorded before retention. No allocation, sorting, unbounded table, new I/O or instrumentation. A compile-time shaped-memo reference switch disables only the new shaped entries while leaving the Step35 zero memo and its storage/clear costs identical.
  - Evidence sufficient to try: Step35's memo image completes current B03 exactly in 37.735005375s and still times out on B10. The actual B10 main-source generator emits 256 `lda #math.VALUE` requests with mode `immediate`, 256 `bne doneNNN` requests with mode `relative`, and 256 zero-shape `nop` requests; module/include statements are outside this count. Because lookup selection uses active-owner precedence, mnemonic and effective mode while execution separately consumes operands, a separate two-entry shaped cache has an ideal static estimate of 510 hits among 512 shaped lookups after two cold fills. This 99.609% estimate is workload evidence only, never a measured hit rate, timeout attribution or predicted gain; a one-entry shaped cache, or sharing the two entries with zero `NOP`, would thrash under this sequence.
  - Cheap screen before expanded proof: first run one same-state B10 reference with only shaped hits/stores disabled, then one shaped candidate, independently, at the unchanged 120000ms post-start/250ms polling boundary and profile-off longword/live/buffered modes. Both use the actual source/package/Rust oracle and fresh fail-closed protocol. If the candidate also times out or exposes no promising completed gain, stop and dispose/revert without timeout ratios or blind retries. A timeout supplies no parity or timing result.
  - Retention proof after positive B10 screen: choose exactly one branch from the same-state screen. If the reference times out and the candidate completes, obtain five exact zero-exit candidate B10 completions total under the censored-baseline rule, retaining the historical and same-state reference timeout evidence without a ratio. If both modes complete, use three alternating matched pairs, counting the screen pair, and require zero mismatches plus median benefit greater than `max(5%, observed range/noise, paired 500ms polling uncertainty/control median)`; this branch does not also require five censored-baseline completions. In either branch run one fresh exact B01 reference/candidate pair with stable per-mode image identity; accept directly when candidate regression relative to reference is no greater than `max(2%, 500ms/reference time)`. If it exceeds that bound, investigate and use at most three alternating B01 pairs to decide non-regression from medians, measured range/noise and the same polling bound; do not blindly repeat. Never compare unlike binaries, modes, packages, or incomplete runs.
  - Required focused proof: extend the actual decoded-package lifecycle oracle and fresh native harness for repeated shaped results, observable two-key result reuse, zero-memo preservation, exact case/oversize fallback, shaped no-match, missing and malformed nonpublication, output-distinct successful selection/reload, failed selection/reload invalidation, and unchanged process/API negative contracts. Require a complete mixed CLI case with exact live Rust artifact, explicit guest outcome and ephemeral files. Source/ABI review alone owns deterministic successful-cold-store round-robin mechanics, corrupt-entry defensive bounds and cold fallback, valid-last publication, eligibility, invalidation and register/CCR/stack behavior; the dynamic immutable-package harness must not mutate private cache or package state, and no implementation-mirroring simulator substitutes for execution.
  - Gate tier: bounded native performance experiment with an early B10 stop/go and mandatory lifecycle/correctness gates before retention.
  - Full quality gates: both candidate/reference host assembly, native formatter, exact source/layout/capacity, focused Level A/B/D proof, malformed controls, staged-native slice, one non-LSP Rust quality gate after focused proof, workflow/artifact/plan validators and independent compliance. Do not restart live runs or rerun broad gates for receipt-only edits. Full corpus/nonterminal qualification remains A-close work.
  - Plan-compliance review evidence: independent PASS must inspect the exact two-entry/key/result/invalidation boundaries, static audit limits, every completed/failed screen, censored-baseline or matched-pair arithmetic, memory/code/stack cost, lifecycle/artifact proofs, gate receipts and preserved debt.
  - Commit outcome: one focused `perf(native): memoize shaped compact lookups` only if the candidate passes its stop/go and final compliance; otherwise revert production and commit an honest no-go result before A-close.
...
- [x] Step 38 · Item A-cli-clear-longword — Test aligned longword CLI clears
  - Source requirement or finding IDs: SR-EARLY, SR-MEAS, SR-PAR; Step37 controlled B10 platform capture records 697,344 frontend clear bytes across 1,362 calls of 512 bytes, without an isolated clear-time measurement. This count authorizes a bounded generic copy primitive experiment only; it does not predict a gain or identify all call-site alignment.
  - Expected files: `native/motorola68000/amigaos/opforge-cli/copy.asm`; the existing production-corpus selector/reference define and diagnostic wrapper; the existing bounded-progress native harness and existing Rust runner/assertions, with no new Rust wiring; measured capacity/inventory metadata; this plan/sidecar, focused slice and results. The integration worktree's reviewed `opforge-cli`→`opforge` rename must be preserved transactionally; do not stage or overwrite unrelated rename files.
  - Dependencies/worktree: Step37 no-go committed at `cc19cbef`; active `AGENTS.md`, plan-authoring and native parity/68000/FS-UAE rules remain binding. This item precedes A-close and does not relax B10, output/frontend/metadata, post-A stop or LSP-last requirements.
  - Rust/native authority: Rust byte-slice clearing and existing native callers define exact cleared ranges. Native `opforge.cli.copy::clearBytes` owns the generic clear primitive and existing platform-profiler callbacks. CPU/family/dialect semantics do not belong in this path.
  - Steps and boundaries: preserve the public ABI exactly: input `A0` pointer and full `D0.L` byte count; zero count may carry a null pointer; return advances `A0` by exactly the original count and leaves `D0=0`; for nonzero counts `D1=0`, while zero count preserves the incoming `D1`; `SP`, `D2-D7` and `A1-A6` remain unchanged. Return CCR is `Z=1,N=V=C=0`; `X` remains unchanged for zero length and is `0` for nonzero length. Preserve the original profiler requested/completed call count, requested byte count and CCR behavior. Use byte operations until `A0` is aligned, a bounded longword loop for groups of four, then a byte tail; never read or write outside the requested range and never dereference zero/null. No CPU semantic names, new storage, call-site specialization or changed counter ownership.
  - Reference isolation: add compile-time `OPFORGE_CLI_CLEAR_BYTE_REFERENCE` that retains the original byte loop while keeping instrumentation, call sites and all surrounding source identical. The corpus environment uses `OPFORGE_CLI_CLEAR=byte|longword` to select reference/candidate explicitly; filenames, stored cases or evidence paths never select an oracle.
  - Cheap screen before expanded proof: after independent source/ABI review and both candidate/reference assemblies pass, run one matched B10 abort-after-one `platform` diagnostic reference followed by candidate using the explicit current manifest, identical case/package/source/argv/profile/modes, 120000ms post-start limit and timer source. Both must complete fresh Level E at the same deliberate guest exit, phase ID, diagnostic, controlled boundary and normalized non-timing counters. Record all elapsed counters but compare only frontend ticks. Continue only if candidate frontend ticks decrease by more than `max(5%, timer quantization/reference ticks)`; otherwise restore and record a no-go without expanded proof or blind retries.
  - Retention timing after a positive screen: run three alternating matched abort-after-one reference/candidate pairs, counting the screen pair. Require zero non-timing drift and candidate median frontend-tick benefit greater than `max(5%, observed range/noise, timer quantization/reference median)`. Report raw ticks and whole-test costs separately; controlled Level E is development-loop evidence, not product timing or parity.
  - Required focused product proof: use exactly nine native matrix rows: zero count/null pointer; offset-1 counts 1, 2 and 3 to exhaust before alignment; `(offset,count)` rows `(0,512)`, `(1,8)`, `(2,8)`, `(3,8)` and `(0,65539)`. Require nonzero surrounding canaries; returned `A0`/`D0`; zero/nonzero `D1`; unchanged `SP`, `D2-D7`, `A1-A6`; exact `Z=1,N=V=C=0`; and `X` unchanged for zero length but `0` for nonzero length, for reference/candidate. The existing bounded-progress harness calls this matrix immediately after clearing its tick and before the first `BeginRun`, including profile-off builds and platform `NO_IO`/`NO_BULK` paths; existing platform-profile tests prove enabled requested/completed calls and bytes. Use the approved harness/progress export framework, not ad-hoc traces or an implementation-mirroring simulator. Retain `native_bounded_progress_fs_uae_confirms_record_contract`, `native_platform_profile_dos_wrappers_keep_one_operation_and_return_state`, platform-profile CLI exact-artifact, output-open-failure and source-capacity contracts.
  - Product/corpus proof after the timing gate: run complete B03 reference and candidate exact live-Rust artifact proofs with stable per-mode native-image identity and explicit guest exit zero. Candidate regression relative to reference must be no greater than `max(2%, 500ms/reference time)`; if exceeded, investigate and use at most three alternating pairs to decide non-regression from medians/noise. Then attempt one current full B10 candidate at the unchanged 120000ms boundary. An incomplete B10 supplies no ratio or parity and remains A-close debt; retention depends on the named repeated controlled-frontend gain plus complete clear-matrix and B03 product proof.
  - Bounded cost: no new persistent storage. Measure source/code size, helper stack and any alignment effect; retain the existing 16-bit control-block and source graph limits. A larger candidate image is acceptable only with the prescribed benefit and proof. Do not infer a speedup from instruction counts or cleared-byte counts.
  - Gate tier: bounded generic native primitive experiment with early completed-diagnostic stop/go and mandatory ABI/product proof before retention.
  - Required focused gates: source/ABI review; candidate/reference assembly and formatter; Level A host matrix expectations and fresh Level D native clear/canary/register/CCR/profile proof; unchanged progress/profile controls; B03 exact reference/candidate proof; measured source/layout/capacity and inventory; staged-native, architecture, workflow/artifact/plan validators. Every native success requires fresh challenge/start/completion, explicit guest exit and ephemeral artifacts; Level E remains non-parity.
  - Full quality gates: one final non-LSP `scripts/workflow/run_rust_quality_gate.sh` if the corpus selector or Rust harness/tests are retained, plus required native/workflow gates and independent compliance before commit. Do not rerun broad gates for receipt-only changes. Full B01–B10 and nonterminal qualification remains A-close work.
  - Plan-compliance review evidence: independent PASS must inspect zero/null and all align/tail paths, exact ABI/CCR and profiler equivalence, reference isolation, controlled-screen arithmetic, clear matrix/canaries, B03 proof, B10 disposition, code/source cost, receipts, integration rename preservation and all retained debt.
  - Commit outcome: one focused `perf(native): clear aligned CLI buffers by longword` only if all stop/go, product proof and compliance pass; otherwise restore production/selector/tests and commit an honest documentation-only no-go before A-close.
  - Definition of done: the clear candidate has a reviewed retain/revert disposition and focused commit; if retained, exact ABI/profile/native matrix and B03 correctness/non-regression pass; B10 and every wider Phase A debt remain open unless their existing closure rules independently pass.

  - Completion evidence (2026-09-08; focused no-go commit pending): reviewed source/ABI and both assemblies pass; fresh controlled Level E reference/candidate frontend ticks are 706/678 (3.9660056657% lower), below strict >5%. The reference outer process overlaps a cached formatter check; guest timing overlap cannot be excluded, adding a comparison limitation. All normalized non-timing counters and diagnostics match. Restore prototype and selector exactly to cc19cbef; no repeated screen, expanded native matrix, B03/full-B10 or full Rust gate for reverted code. The Step38 no-go MD/JSON retains receipts and limitations. No retained gain or debt closure; final workflow/compliance and focused commit required.

- [x] Step 39 · Item A-session-tail — Reset all session storage, including the final six presence bytes
  - Source requirement or finding IDs: SR-PAR; recorded Step09/Step10/Step13 six-byte initialization debt. Active `AGENTS.md` and native parity/68000/FS-UAE/workflow rules remain binding.
  - Expected files: `native/motorola68000/amigaos/opasm/opasm_engine.asm`; `scripts/performance/tests/test_session_clear.py` and `test_statement_initialization.py`; focused `test-harnesses/debug/opasm_session_init_harness.asm`; existing Rust test/FS-UAE wiring; source inventory/formatter metadata only as required; plan/sidecar, slice and evidence.
  - Dependencies/worktree: Step38 committed at `f233b99e`; implement in the warm worktree and integrate locally while preserving the pending CLI rename. No push. This item precedes A-close; all other debt and the Phase A stop remain unchanged.
  - Rust/native authority: `crates/opforge-types/src/image.rs::ImageStore::new` starts with no written entries. Native `opasm_engine.asm::initSessionV1` resets session state and its image-presence map. The header emits 98 bytes but its factored clear count uses 92; both full and live-statement initialization leave the final six presence bytes untouched.
  - First concrete code change: set the public factored header count to 98; derive the internal emitted-header alias from it and remove comments preserving the discrepancy. Keep all allocations, field offsets, capacities, clear primitives and statement-liveness behavior unchanged. Total requested session extent increases by exactly six bytes to 41,221,934; this is a correctness fix, not a speed claim.
  - Boundary contract: initialize through the actual `opasmEngineAssemblySessionEnd` exactly. Full mode clears the complete session; default live mode clears prefix/suffix while leaving unused statement tables untouched and clearing each newly live row as before. D0 success, CPU-name copy, preserved registers/stack and profiler ownership remain unchanged. Do not add production instrumentation or test-specific production APIs.
  - Focused host proof: update the existing source-driven Level C models to require computed clear end equals the emitted session end, final six bytes zero, outside-session guards intact, unused live-mode statement poison intact, and full-mode zeroed statements. Preserve existing alignment/register/stack/CCR and boundary-index checks. Host models prove modeled memory effects only; source/assembly layout provides Level B extent evidence, not native execution.
  - Focused native proof: add a minimal permanent-contract harness using existing exported engine state/accessors, poison the actual final six presence bytes before `initSessionV1`, and assert zero afterwards; repeat with a different poison to prove reuse. Check an outside-session prefix sentinel, CPU-name/status and representative unused statement storage (preserved live, cleared full). Do not write outside allocated memory to manufacture a trailing guard. Pair source-derived exact-end/guard proof with these actual-native assertions. Assemble and execute both live and full-statement modes with the existing fail-closed fresh challenge/start/completion/explicit zero-exit/ephemeral cleanup runner, attempting the second mode even if the first fails. No Level C simulator substitutes for Level D. The existing byte/longword primitive behavior is unchanged.
  - Product proof: one complete current profile-off B01 CLI run with exact live Rust artifact parity and explicit guest exit zero; report its elapsed time as a current observation, never infer a step gain from unmatched historical timings. Six more cleared bytes imply no claimed performance benefit. Preserve the historical cumulative tally and its comparability limitations.
  - Required focused gates: initialization and clear Python suites, candidate native harness/CLI assembly, exact layout/source capacity, focused Level D both modes and B01, native formatter, architecture/inventory/staged slice, workflow/artifact/plan validators and independent compliance.
  - Full quality gates: one final non-LSP `scripts/workflow/run_rust_quality_gate.sh` for retained Rust wiring after focused proof. Do not restart a live gate or rerun broad tests for receipt-only edits. Full corpus/current nonterminal qualification remains at A-close.
  - Plan-compliance review evidence: independent PASS must verify the exact extent correction, no allocation/offset/liveness drift, final-six and reuse native proof in both modes, host bounds/guards, fresh product proof, required receipts and preserved wider debt/stop boundary.
  - Commit outcome: one focused `fix(native): reset the complete session presence map` after all required gates and compliance pass; no next item before commit.
  - Definition of done: the named six-byte initialization debt is repaired and proven in both modes, committed and safely integrated; no other correctness debt or total performance qualification is claimed closed.
  - Completion evidence (2026-09-08; focused commit and safe integration follow this recorded evidence): `documentation/performance/results/opforge-step39-session-tail-2026-09-08.md` and `.json` record the exact 92-to-98 header repair and 41,221,934-byte session extent, unchanged allocation/offset/live-row behavior, 12 host model checks, fresh live/full native reuse proof with explicit zero exits, current exact B01 parity, six capacity checks, final non-LSP Rust, staged-native, inventory, workflow, artifact and plan PASS receipts, and independent compliance PASS. The native pair timeout-variable deviation is disclosed and accepted only as correctness proof because the entire two-guest process completed within 120 seconds. No step speed gain, stale-output cause, B10 result, wider debt closure or total Phase A gain is claimed.

- [ ] Step 40 · Item A-close — Qualify early wins and clear the recorded failure debt
  ...
  - Additional recorded section-start debt: Step30 source review confirms native unplaced sections first encountered under a nonzero parent PC retain that pass-one start, whereas Rust begins an unplaced section at zero. Step30 start-zero BIN cases and retained-start convergence correction do not close this pre-existing boundary; any repair needs its own numbered, reviewed item before A-close.
  - Additional recorded symbol metadata debt: native mutable-symbol kind and non-root public visibility require authoritative retained metadata and complete proof before A-close; root immutable listing fixtures do not close them.
...
- [ ] Step 41 · Item C0 — Dispose of the old roadmap and select only residual winners
...
- [ ] Step 42 · Item P0 — Finish profile foundations after the early product wins
...
- [ ] Step 43 · Item P-close — Verify profile coverage and final hotspot decisions
  - Source requirement or finding IDs: SR-ID, SR-MEAS, SR-ACC, F11.
...
- [ ] Step 44 · Item 27 — Install regression budgets for accepted mechanisms
...
- [ ] Step 45 · Item B-close — Qualify residual optimizations and observability
...
- [ ] Step 46 · Item 28 — Qualify optimized and reference modes before cleanup
...
- [ ] Step 47 · Item 29 — Schedule and execute only cleanup that actually exists
  - Source requirement or finding IDs: SR-PAR, SR-ACC, SR-TERM.
...
- [ ] Step 48 · Item 30 — Prove the final performance production shape
...
- [ ] Step 49 · Item LSP-close — Repair and qualify LSP at the very end
...
- [x] Step 1 · Item 1 — Activate on the latest remote self-host parking checkpoint and publish the performance ledger
...
- [x] Step 2 · Item 0a — Add bounded native progress and coarse phase timing
...
- [x] Step 3 · Item 0b — Count native pass, statement, flow, and layout multiplication
...
- [x] Step 4 · Item 0c — Count native symbol and expression work
...
- [x] Step 5 · Item 0d — Count coarse native VM and service execution
...
- [x] Step 6 · Item 0e — Count native platform I/O, clear, and copy work
...
- [x] Step 7 · Item 2 — Freeze the production-path benchmark corpus and result protocol
