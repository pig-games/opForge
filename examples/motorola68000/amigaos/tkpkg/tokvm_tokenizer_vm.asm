...
TK_KIND_OP_LOGIC_AND            = 30
TK_KIND_OP_LOGIC_OR             = 31
TK_KIND_OP_LOGIC_XOR            = 32
TK_KIND_OP_EQ                   = 33
TK_KIND_OP_NE                   = 34
TK_KIND_OP_GE                   = 35
TK_KIND_OP_GT                   = 36
...
tokvmProgramLoop:
        ...
        MOVE.L LOCAL_STEP_COUNT(A2), D0
        CMPI.L #2, D0
        BNE.S tokvmProgramLoopDispatchOpcode
        MOVE.W #TK_VM_FAILURE_KIND_FAIL, tokvmLastFailureKind
        MOVE.W D1, tokvmLastFailureOperand
...
; Jump operands are encoded little-endian in the VM bytecode because the Rust
; builder writes u32 targets with to_le_bytes(). The 68020 reads longwords as
; big-endian, so these rotations perform the byte swap before converting the
; offset back into an A3-relative program counter.
tokvmOpcodeJump:
        MOVE.L A0, D0
        SUB.L A3, D0
        ADDQ.L #4, D0
...
tokvmOpcodeJumpIfClass:
        ...
        CMPI.B #1, D0
        BEQ tokvmClassWhitespace
        CMPI.B #2, D0
        BEQ tokvmClassIdentStart
        CMPI.B #3, D0
        BEQ tokvmClassIdentContinue
        CMPI.B #4, D0
        BEQ tokvmClassDigit
        CMPI.B #5, D0
        BEQ tokvmClassQuote
...
tokvmScanStringLoop:
        ...
        MOVEQ #0, D0
        MOVE.B 0(A4, D2.L), D0
        CMP.B LOCAL_CURRENT_BYTE(A2), D0
...
tokvmScanStringEmitDecoded:
        ...
        CMP.L D6, D1
        BCC tokvmPendingLexemeOverflowFromScan
        MOVE.B D0, (A0)+  ; store decoded payload bytes, not the surrounding quotes/escape markers
        ADDQ.L #1, LOCAL_PENDING_LEX_LEN(A2)
        ADDQ.L #1, D2
...
tokvmScanStarLike:
        ; The native demo accepts '*' as multiply but treats '**' as invalid
        ; program input because power is not implemented in this first slice.
        ADDQ.L #1, D2
        CMP.L D4, D2
        BCC tokvmStageMultiply
        CMPI.B #'*', 0(A4, D2.L)
        BEQ tokvmScanSymbolInvalidProgram
...
tokvmScanCaretLike:
        ; '^' is bitwise xor/caret, while '^^' is promoted to the logical xor
...
tokvmStageCaret:
        MOVE.W #TK_KIND_OP_CARET, LOCAL_PENDING_KIND(A2)
        LEA lexCaret, A0
        MOVEQ #1, D0
        BRA tokvmStageAndCommitSymbol
