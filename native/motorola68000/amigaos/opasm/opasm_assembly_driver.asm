...
; Detect fixed-size 3-byte bit-branch mnemonics (BBR0-BBR7 / BBS0-BBS7).
; Inputs: A0 = mnemonic text, D0.W = mnemonic length.
; Outputs: D0 = 1 when the mnemonic is a 3-byte bit branch, else 0.
; Clobbers: D1-D2/A0/CCR.
; CCR: reflects D0 on return.
opasmDriverIsBitBranchMnemonic	.block
	cmpi.w #4, d0
	bne.s no
	move.b (a0)+, d1
	ori.b #$20, d1
...
no
	moveq #0, d0
	rts
	.bend  ; opasmDriverIsBitBranchMnemonic

; Detect fixed-size 8-bit relative branch mnemonics for pass-one sizing.
; Inputs: A0 = mnemonic text, D0.W = mnemonic length.
; Outputs: D0 = 1 when the mnemonic is a relative branch, else 0.
; Clobbers: D1-D2/A0/CCR.
; CCR: reflects D0 on return.
opasmDriverIsRelativeBranchMnemonic	.block
	cmpi.w #3, d0
	beq.s lenOk
	moveq #0, d0
	rts
...
yes
	moveq #1, d0
	rts
	.bend  ; opasmDriverIsRelativeBranchMnemonic

; Detect accumulator operand text `a` / `A` after statement operand trimming.
; Inputs: A0 = operand text, D0.W = operand length.
; Outputs: D0 = 1 when the operand is accumulator A, else 0.
opasmDriverOperandIsAccumulatorA	.block
	cmpi.w #1, d0
	bne.s no
	move.b (a0), d1
	ori.b #$20, d1
...
no
	moveq #0, d0
	rts
	.bend  ; opasmDriverOperandIsAccumulatorA

; Detect MOS accumulator-shape mnemonics that Rust selects as one-byte forms.
; Inputs: A0 = mnemonic text, D0.W = mnemonic length.
; Outputs: D0 = 1 when mnemonic + operand A should size as one byte.
opasmDriverIsAccumulatorOneByteMnemonic	.block
	cmpi.w #3, d0
	bne.w no
	move.b (a0)+, d1
	ori.b #$20, d1
...
no
	moveq #0, d0
	rts
	.bend  ; opasmDriverIsAccumulatorOneByteMnemonic
