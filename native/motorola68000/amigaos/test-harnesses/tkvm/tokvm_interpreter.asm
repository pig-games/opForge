...
start	.block
	...
	movea.l cli_harness.SYS_BASE.W, a6  ; Exec base for FindTask/WaitPort/GetMsg/ReplyMsg/Forbid
	jsr cli_harness.FIND_TASK(a6)

	movea.l d0, a2
	tst.l cli_harness.PR_CLI(a2)  ; tokvm's first native slice only supports Shell launches, not Workbench icons
	bne.w cli

	lea cli_harness.PR_MSG_PORT(a2), a0
	jsr cli_harness.WAIT_PORT(a6)
	lea cli_harness.PR_MSG_PORT(a2), a0
	jsr cli_harness.GET_MSG(a6)
	move.l d0, d2  ; cache the startup message so we can ReplyMsg before returning
	moveq #cli_harness.RETURN_WORKBENCH_UNSUPPORTED, d7  ; host-visible status for unsupported Workbench activation
	bra.w reply

cli
	jsr cli_harness.tokvmAmigaosCliHarnessRun  ; hand off to the module that mirrors the Rust host/report bridge
...
reply
	...
	beq.w done
	jsr cli_harness.FORBID(a6)
	movea.l d2, a1
	jsr cli_harness.REPLY_MSG(a6)
