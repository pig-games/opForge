...
usage() {
  cat <<'EOF'
Usage:
  scripts/workflow/run_rust_quality_gate.sh [--defer-lsp] [--help]
...
Runs the canonical Rust quality gate for opForge code changes:
  ...
  4. cargo fmt --all
  5. cargo clippy "${cargo_scope[@]}" -- -D warnings
  6. cargo audit
...
cargo_scope=()
defer_lsp=0
if [[ $# -eq 1 && "$1" == "--defer-lsp" ]]; then
  cargo_scope=(--workspace --exclude lsp)
  defer_lsp=1
elif [[ $# -ne 0 ]]; then
  usage >&2
  exit 1
...
run_step "Run Rust clippy" cargo clippy "${cargo_scope[@]}" -- -D warnings
run_step "Run cargo audit" cargo audit
run_step "Require C compiler for FFI ABI coverage" require_c_compiler
run_step "Run locked Rust test suite" cargo test "${cargo_scope[@]}" --locked

if [[ $defer_lsp -eq 1 ]]; then
  printf '\nPASS: non-LSP Rust quality gate complete; lsp clippy/tests DEFERRED, not full-workspace qualification.\n'
else
